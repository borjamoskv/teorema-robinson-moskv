...
def test_bpe_blindness_prevention() -> None:
    target_word = "diecisiete"
    target_char = "e"
    count = count_letters_deterministically(target_word, target_char)
    assert count == 3, (
        f"Failed deterministic check: {target_word} count of {target_char} is not 3"
    )
    ...
    with sqlite3.connect(DB_PATH, timeout=5.0) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT tokens FROM exergy_ledger WHERE causal_hash = ?",
            (node.causal_hash,),
        )
        res = cursor.fetchone()
