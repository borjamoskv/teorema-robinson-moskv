...
def event_factory():
    ...
    def _create(idx: int):
        return LedgerEvent(stream='test_stream', entity_id=f'ent_{idx}', event_type='CREATED', payload={'foo': 'bar', 'idx': idx}, cortex_taint='test_taint', source_db='test_db', source_table='test_tbl', source_pk=f'pk_{idx}')
    return _create
    ...
    try:
        ...
        async with aiosqlite.connect(db_path) as db:
            with pytest.raises(sqlite3.IntegrityError, match='immutable master ledger'):
                await db.execute("UPDATE ledger_entries SET payload_json = 'foo' WHERE seq = ?", (seq,))
            with pytest.raises(sqlite3.IntegrityError, match='immutable master ledger'):
