...
async def test_ledger_append_and_idempotency(db_path: Path, event_factory):
    ...
    try:
        ...
        # 2. Idempotency Masking (INV_BFT_05)
        # Re-appending the exact same event should yield the identical result silently
        ...
        # 3. Zombie Actor Check (INV_BFT_07)
        await actor.stop()
...
async def test_immutable_triggers(db_path: Path, event_factory):
    ...
    try:
        ...
        # Attempt to mutate the ledger physically
        async with aiosqlite.connect(db_path) as db:
