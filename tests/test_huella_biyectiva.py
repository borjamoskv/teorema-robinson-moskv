...
YAML_PATH = str(
    ...
    / "cortex"
    / "ontology"
    / "huella_biyectiva_hardware.yaml"
)
...
def test_isomorphisms_density():
    ...
    assert isinstance(mappings, list), (
        "Topologia_Isomorfica_Borja_Moskv debe ser una lista."
    )
    assert len(mappings) >= 18, (
        f"Se esperaban al menos 18 mapeos de huella hardware, se encontraron {len(mappings)}"
    )
    for i, mapping in enumerate(mappings):
        assert "Vector_Origen" in mapping, f"Falta Vector_Origen en el mapeo índice {i}"
        assert "Vector_Destino" in mapping, (
            f"Falta Vector_Destino en el mapeo índice {i}"
        )
...
def test_cryptographic_signature():
    ...
    assert "[CORTEX-TAINT:" in signature.get("Atribucion", ""), (
        "Falta la firma CORTEX-TAINT en atribución."
    )
    assert signature.get("ZKP_Assertion") is not None, (
        "Falta la declaración zkProof/BLAKE3."
    )
