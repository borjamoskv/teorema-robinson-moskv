...
if not os.path.exists(DB_PATH):
    ...
    DB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "cortex_memory.db"))

def parse_td(s):
    m = re.match(r'T(\d+)\.D(\d+)', s)
    if m:
...
def test_assert_cyclic_dimensional_homomorphism_invariant():
    """
    Asserts that the 36 Dimensional edges in cortex_memory.db follow the cyclic
    shift rule: T_tgt = (T_src + 3) mod 9 and D_tgt = (D_src + 2) mod 8.
    """
    ...
    cursor.execute("SELECT source, target, type FROM L2_isomorphism_edges WHERE type = 'Dimensional'")
    ...
    for e in edges:
        ...
        # Verify Shift T: +3 (mod 9)
        expected_t = (t_src + 3) % 9
        if expected_t == 0:
            expected_t = 9
        assert t_tgt == expected_t, f"Theory shift mismatch: {src} -> {tgt} (Expected T={expected_t}, got T={t_tgt})"
        
        # Verify Shift D: +2 (mod 8)
        expected_d = (d_src + 2) % 8
        if expected_d == 0:
            expected_d = 8
        assert d_tgt == expected_d, f"Dimension shift mismatch: {src} -> {tgt} (Expected D={expected_d}, got D={d_tgt})"

def test_assert_local_primitive_nodes_bijections():
    """
    Asserts that each dimensional mapping pairs exactly 10 nodes with ordered bijection
    by sorting their canonical IDs.
    """
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    ...
    cursor.execute("SELECT source, target FROM L2_isomorphism_edges WHERE type = 'Dimensional'")
    dim_edges = [dict(row) for row in cursor.fetchall()]
    conn.close()
    
    # Group nodes by theory-dimension
    td_nodes = {}
    for n in nodes:
        nid = n['id']
        if '.' in nid:
    ...
    for edge in dim_edges:
        ...
        assert len(src_list) == 10, f"Source group {src} has {len(src_list)} nodes instead of 10."
        assert len(tgt_list) == 10, f"Target group {tgt} has {len(tgt_list)} nodes instead of 10."
