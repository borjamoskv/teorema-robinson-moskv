...
if not os.path.exists(ENGINE_YAML_PATH):
    print(f'\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m Essential Config Missing: {ENGINE_YAML_PATH}. C5-REAL Fail-Fast.', file=sys.stderr)
    sys.exit(1)
with open(ENGINE_YAML_PATH, 'r', encoding='utf-8') as f:
    ENGINE_CONFIG = yaml.safe_load(f)

class CortexInferenceEngine:
    TRIGGERS = {'causal': re.compile('\\b(causar|provocar|generar|hacer|por qué|efecto)\\b', re.IGNORECASE), 'mereo': re.compile('\\b(parte|sistema|estructura|composición|dividir)\\b', re.IGNORECASE), 'process': re.compile('\\b(cambiar|evolucionar|fluir|transitar|dinámica|tiempo)\\b', re.IGNORECASE), 'modal': re.compile('\\b(podría|debería|sería|quizás|posible|mundo)\\b', re.IGNORECASE), 'info': re.compile('\\b(mejorar|optimizar|aprender|entrenar|divergencia|entropía)\\b', re.IGNORECASE), 'semiotic': re.compile('\\b(significar|interpretar|leer|texto|signo|código)\\b', re.IGNORECASE), 'epistemic': re.compile('\\b(confianza|verdad|verificar|test|hash|isomorfismo)\\b', re.IGNORECASE)}
    ...
    async def retrieve_primitives(self, mode, scores, query=''):
        theory_map = {'MODE-01-CAUSAL-DEDUCTION': ['CAUSAL-ONTOLOGY', 'GRAPH-MORPHISMS'], 'MODE-02-MEREOTOPOLOGICAL-COMPOSITION': ['MEREOTOPOLOGY', 'SYSTEMIC-BOUNDARIES', 'GRAPH-MORPHISMS'], 'MODE-03-PROCESS-DYNAMICS': ['PROCESS-DYNAMICS', 'INFORMATION-GEOMETRY', 'SYSTEMIC-BOUNDARIES'], 'MODE-04-MODAL-EXPLORATION': ['MODAL-SPACES', 'COMPUTATIONAL-STATES', 'INTENTIONAL-STRUCTURES'], 'MODE-05-INFORMATION-GEOMETRY': ['INFORMATION-GEOMETRY', 'COMPUTATIONAL-STATES', 'GRAPH-MORPHISMS'], 'MODE-06-SEMIOTIC-DECODING': ['SEMIOTIC-ENCODING', 'INTENTIONAL-STRUCTURES', 'MODAL-SPACES'], 'MODE-07-EPISTEMIC-TRUST': ['EPISTEMIC-BOUNDARY', 'ASYMMETRIC-TRUST', 'GRAPH-MORPHISMS']}
    ...
    async def execute_inference(self, query):
        ...
        if cached:
            ...
            await self.ledger.record_transaction_async(project='CORTEX_INFERENCE_L3', action='CACHE_HIT_BYPASS', detail={'query_hash': query_hash}, tenant_id='inference_engine')
            return trace_data
        scores = self.parse_query(query)
        sorted_modes = sorted(scores.items(), key=lambda x: x[1], reverse=True)
        max_category = sorted_modes[0][0]
        mode_mapping = {'causal': 'MODE-01-CAUSAL-DEDUCTION', 'mereo': 'MODE-02-MEREOTOPOLOGICAL-COMPOSITION', 'process': 'MODE-03-PROCESS-DYNAMICS', 'modal': 'MODE-04-MODAL-EXPLORATION', 'info': 'MODE-05-INFORMATION-GEOMETRY', 'semiotic': 'MODE-06-SEMIOTIC-DECODING', 'epistemic': 'MODE-07-EPISTEMIC-TRUST'}
        ...
        if active_mode not in self.config['inference_modes']:
            print(f'\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m YAML Key missing for mode {active_mode}. C5-REAL Fail-Fast.', file=sys.stderr)
            sys.exit(1)
        ...
        def decimal_to_str(obj):
            if isinstance(obj, dict):
                ...
                return float(obj)
            return obj
        trace = {'claim': f'Resolución de inferencia en modo {active_mode}', 'proof': {'Base': 'Teorema-Robinson-Moskv / CORTEX-db', 'Confidence': confidence}, 'query': query, 'mode_activated': active_mode, 'activation_vector': decimal_to_str(scores), 'retrieved_nodes': [p['id'] for p in primitives], 'applied_isomorphisms': [i['id'] for i in isomorphisms], 'reasoning_steps': steps}
        if active_mode == 'MODE-07-EPISTEMIC-TRUST':
        ...
        await self.cache_db.execute('\n            INSERT INTO L3_inference_cache \n            (query_hash, active_mode, retrieved_nodes, applied_isomorphisms, trace_payload, hits)\n            VALUES (?, ?, ?, ?, ?, 0)\n            ON CONFLICT(query_hash) DO UPDATE SET\n                active_mode = excluded.active_mode,\n                retrieved_nodes = excluded.retrieved_nodes,\n                applied_isomorphisms = excluded.applied_isomorphisms,\n                trace_payload = excluded.trace_payload\n            ', (query_hash, active_mode, json.dumps(trace['retrieved_nodes'], ensure_ascii=False), json.dumps(trace['applied_isomorphisms'], ensure_ascii=False), stored_payload))
        await self.cache_db.commit()
        await self.ledger.record_transaction_async(project='CORTEX_INFERENCE', action='CACHE_MISS_EVALUATED', detail={'query_hash': query_hash, 'mode_activated': active_mode}, tenant_id='inference_engine')
        return trace
