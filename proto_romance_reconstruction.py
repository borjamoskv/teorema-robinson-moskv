...
# -*- coding: utf-8 -*-
"""
Reconstrucción proto-lingüística PROBABILÍSTICA — versión honesta y validable.

Qué hace (y qué NO hace):
  - Toma cognados reales de 6 lenguas romances y reconstruye el proto-segmento
    de cada posición como una DISTRIBUCIÓN de probabilidad (no un token único).
  - Usa el mismo aparato matemático que la filogenética bayesiana de lenguas
    (Gray & Atkinson; Bouckaert et al.): un modelo de sustitución de Markov
    sobre segmentos + poda de Felsenstein sobre un árbol fijo.
  - VALIDA contra el latín atestiguado (ground truth), porque el proto del
    romance SÍ lo conocemos. Esto convierte la afirmación en falsable.
  - CUANTIFICA la incertidumbre (entropía por posición) y muestra que las
    posiciones donde el cambio fonético destruyó información (p.ej. fusiones)
    son exactamente las de mayor entropía y mayor error. Eso es la
    irreversibilidad, medida — no una única raíz "sin ambigüedad".

NO reconstruye una lengua "Pre-Babel" única ni un "lenguaje del cerebro
primitivo". Eso no es recuperable: el cambio fonético es many-to-one y borra
información. Aquí se estima una distribución posterior, con su error real.

Transcripción: fonética ASCII coarse (estilo ASJP). Geminadas, vocales nasales
y redondeadas anteriores están simplificadas — es una muestra ilustrativa; el
...
# ----------------------------------------------------------------------------
# 1) DATOS: cognados romances (daughters) + latín atestiguado (GOLD)
...
# ----------------------------------------------------------------------------
# 2) RASGOS FONÉTICOS -> distancia -> intercambiabilidad del modelo de Markov
# ----------------------------------------------------------------------------
    ...
    for i in range(S):
        for j in range(S):
            if i != j:
                Q[i, j] = R[i, j] * pi[j]
        Q[i, i] = -np.sum(Q[i, :])
    # normaliza a 1 sustitución esperada por unidad de rama
...
def main():
    ...
    # (a) MSA de cada cognado (solo daughters) y recuento estacionario pi
    msas = {}
    counts = {}
    ...
    # correlación entropía vs error (punto-biserial)
    es = np.array([e for e, _ in all_ent_match])
    fs = np.array([0.0 if f else 1.0 for _, f in all_ent_match])  # 1 = error
