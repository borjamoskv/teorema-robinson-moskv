...
class SolidityHookAuditor:
    ...
    def audit_contract_code(cls, code: str, name: str = "InMemory") -> Dict[str, Any]:
        ...
        for func in cls.V4_CALLBACK_FUNCTIONS:
            func_pattern = (
                f"function\\s+{func}\\s*\\([^)]*\\)[^{{]*\\{{([\\s\\S]*?)\\}}"
            )
            for match in re.finditer(func_pattern, clean_code):
                checks_performed += 1
                func_body = match.group(1)
                has_pool_manager_check = (
                    "onlyPoolManager" in match.group(0)
                    or "msg.sender == poolManager" in func_body
                    or "msg.sender != poolManager" in func_body
                    or ("address(poolManager)" in func_body)
                )
        ...
        has_state_mutation = (
            "balances[" in clean_code
            or "fees[" in clean_code
            or "tstore(" in clean_code
        )
                ...
                for match in re.finditer(func_pattern, clean_code):
                    ...
                    if "balances[" in func_body or "fees[" in func_body:
                        has_pool_id_validation = (
                            "authorizedPools" in func_body
                            or "authorizedPools[" in clean_code
                            or ("require(" in func_body and "poolId" in func_body)
                        )
        ...
        if "executeAction" in clean_code or "swap(" in clean_code:
            ...
            if not has_tstore and (
                "balances[" in clean_code or "reentrancy" in clean_code.lower()
            ):
                violations.append(
                    {
                        "invariant": "INV-03",
    ...
    try:
        for finding in findings:
            ...
            if audit.get("is_vulnerable"):
                ...
                extraction_results.append(
                    {"target": finding["target"], "vulnerable": False}
                )
    finally:
        await actor.stop()
    return {
