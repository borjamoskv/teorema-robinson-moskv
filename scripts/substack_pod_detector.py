...
def analyze_interactions(interactions):
    ...
    for user in all_users:
        ...
        anomalies.append({'user': user, 'out_count': len(interacted_with), 'in_count': len(received_from), 'reciprocal_count': len(overlap), 'reciprocity_ratio': round(overlap_ratio, 2), 'entropy': round(ent, 3), 'coordination_score': round(coordination_score, 2), 'is_anomaly': coordination_score > 3.0 and ent < 4.0})
    return {'total_users': len(all_users), 'reciprocal_pairs_count': len(reciprocal_pairs), 'reciprocal_pairs': reciprocal_pairs, 'anomalies': sorted(anomalies, key=lambda x: x['coordination_score'], reverse=True)}
