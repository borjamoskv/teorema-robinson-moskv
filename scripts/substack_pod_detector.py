# -*- coding: utf-8 -*-
"""
Substack Engagement Pod Detector (C5-REAL Standard)
Computes network reciprocity, density, and temporal clustering anomalies to expose coordinated circles.
"""
import sys
import json
import math
...
def analyze_interactions(interactions):
    """
    interactions: list of dict with keys:
      - 'timestamp': epoch seconds
      - 'author': account posting the note
    ...
    # Temporal clustering per note
    author_notes_timing = defaultdict(list) # author -> list of interactor timestamps
    
    for inter in interactions:
        u1, u2 = inter['author'], inter['interactor']
        all_users.add(u1)
        all_users.add(u2)
        out_degrees[u2].add(u1) # u2 interacts with u1
        in_degrees[u1].add(u2)  # u1 receives from u2
    ...
    return {
        ...
        "anomalies": sorted(anomalies, key=lambda x: x['coordination_score'], reverse=True)
    }

if __name__ == "__main__":
    # Mock data showing a tight coordination loop
    mock_data = []
    pod_members = ["david_crecer", "user_A", "user_B", "user_C", "user_D", "user_E"]
    for member in pod_members:
        for target in pod_members:
            if member != target:
                mock_data.append({"author": target, "interactor": member, "timestamp": 1720720000, "type": "comment"})
                mock_data.append({"author": target, "interactor": member, "timestamp": 1720720010, "type": "like"})

    organic = ["organic_user_1", "organic_user_2", "organic_user_3"]
    for org in organic:
        mock_data.append({"author": "david_crecer", "interactor": org, "timestamp": 1720731000, "type": "comment"})
