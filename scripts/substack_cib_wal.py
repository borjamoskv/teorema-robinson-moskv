...
class SubstackCIBWALDetector:
    ...
    def _append_ledger(self, conn: sqlite3.Connection, action: str) -> str:
        ...
        current_hash = hashlib.sha256(f'{prev_hash}{action}{cortex_taint}'.encode()).hexdigest()
        cur.execute('INSERT INTO master_ledger (action, prev_hash, current_hash, cortex_taint, timestamp) VALUES (?, ?, ?, ?, ?)', (action, prev_hash, current_hash, cortex_taint, timestamp))
        return current_hash

    def ingest_interaction(self, source_node: str, target_node: str, interaction_type: str):
        timestamp = time.time()
        cortex_taint = f'INGEST|{timestamp}|borjamoskv'
        action_log = f'INGEST: {source_node} -> {target_node} [{interaction_type}]'
        with sqlite3.connect(self.db_path, timeout=5.0) as conn:
            conn.execute('INSERT OR IGNORE INTO cib_nodes (node_id, cortex_taint) VALUES (?, ?)', (source_node, cortex_taint))
            conn.execute('INSERT OR IGNORE INTO cib_nodes (node_id, cortex_taint) VALUES (?, ?)', (target_node, cortex_taint))
            conn.execute('INSERT INTO cib_edges (source, target, type, timestamp, cortex_taint) VALUES (?, ?, ?, ?, ?)', (source_node, target_node, interaction_type, timestamp, cortex_taint))
            self._append_ledger(conn, action_log)

    def calculate_roi_dissonance(self, node_id: str, estimated_hours_spent: float) -> str:
        with sqlite3.connect(self.db_path, timeout=5.0) as conn:
            cur = conn.cursor()
            ...
            return json.dumps({'node_id': node_id, 'opportunity_cost_usd': opportunity_cost, 'organic_subs_gained': subs_gained_from_pod, 'verdict': verdict, 'ledger_hash': hash_val}, indent=2)
