...
def analyze_cib_network(interactions, anonymize=False):
    ...
    for i in range(len(interactors_list)):
        for j in range(i + 1, len(interactors_list)):
            ...
            if j_score > 0.6:
                coordination_links.append({'user_a': u1, 'user_b': u2, 'jaccard_similarity': round(j_score, 3)})
    user_profiles = []
    for user in all_interactors:
        ...
        user_profiles.append({'user': user, 'total_interactions': total_interactions, 'temporal_entropy': round(entropy, 3), 'coordination_anomaly_score': round(anomaly_score, 3)})
    return {'coordination_links': sorted(coordination_links, key=lambda x: x['jaccard_similarity'], reverse=True), 'anomalies': sorted(user_profiles, key=lambda x: x['coordination_anomaly_score'], reverse=True)}
if __name__ == '__main__':
    ...
    for note_idx in range(20):
        ...
        for idx, member in enumerate(cohort):
            ...
            mock_interactions.append({'note_id': note_id, 'author': 'david_crecer', 'interactor': member, 'timestamp': ts})
    for org_idx in range(5):
        org_name = f'organic_{org_idx}'
        for note_idx in [2, 5, 12]:
            ts = 1720720000 + note_idx * 3600 + 1200 + org_idx * 600
            mock_interactions.append({'note_id': f'note_{note_idx}', 'author': 'david_crecer', 'interactor': org_name, 'timestamp': ts})
