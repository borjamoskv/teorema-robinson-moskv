#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# MOSKV-1 APEX: OBJECTIVES & MILESTONES AGENT (C5-REAL)
"""
Agente autónomo para la gestión de objetivos y hitos (Milestones) de Babylon-60.
Enfuerza las directivas:
- R2 (Justificación Densa YAML)
- R4 (Git Sentinel commit autónomo)
- Ω15 (Idempotency Lock para ahorro de ATP)
...
def save_state(state: Dict[str, Any]) -> bool:
    # Remove old CORTEX_TAINT to calculate hash on current logical payload
    state.pop("CORTEX_TAINT", None)
    
    # Dump to canonical string to check idempotency and calculate taint
...
def update_project_md(state: Dict[str, Any]) -> bool:
    ...
    # Split content by ## Milestones
    if "## Milestones" not in content:
        # Append if not found
        new_content = content + "\n" + generate_milestones_table(state)
    else:
        parts = content.split("## Milestones")
        # Find next header in the second part if any, to preserve post-milestone sections
        post_part = parts[1]
...
def cmd_add_milestone(args):
    ...
    new_ms = {
        "id": ms_id,
        "title": args.title,
        "due": args.due or datetime.now(timezone.utc).date().isoformat(),
        "status": "PENDING",
        "commit_hash": args.hash or ""
    }
...
def cmd_iter(args):
    """
    ULTRAThink ITERA:
    - Escanea los tags y el log de git.
    - Sincroniza hitos pendientes si detecta commit_hashes correspondientes.
    - Sincroniza PROJECT.md.
    - Computa exergía general de los objetivos basándose en la tasa de finalización de milestones.
    - Guarda el estado y ejecuta commit Git Sentinel.
    """
    state = load_state()
    ...
    for o in state.get("objectives", []):
        ...
        for m in o.get("milestones", []):
            # Si el milestone está pendiente pero tiene commit hash, verificar si está en la historia de git
        ...
        # Calcular puntuación de exergía (done / total o exergía base ponderada)
        if total_ms > 0:
