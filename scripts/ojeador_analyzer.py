...
SESSION_ARTIFACT_PATH = f'$CORTEX_ROOT/.gemini/antigravity/brain/{current_session_id}/ojeador_arena_matrix.md'
...
BIAS_REGISTRY = {'claude': {'family': 'Claude', 'alignment_risk': 'Moderate-High (Refusal on copyright, strict)', 'strengths': 'Logic, structured code', 'weaknesses': 'Sycophancy under personas', 'exergy_rating': 'A'}, 'gpt-4': {'family': 'GPT-4', 'alignment_risk': 'High (Sycophancy, intense reward hacking)', 'strengths': 'General instructions, fast drafting', 'weaknesses': 'Highly verbose, green theater', 'exergy_rating': 'B'}, 'gemini': {'family': 'Gemini', 'alignment_risk': 'High (Rigid safety filtering on sensitive topics)', 'strengths': 'Extreme context window, NIAH', 'weaknesses': 'Semantic friction, aggressive refusals', 'exergy_rating': 'B+'}, 'llama': {'family': 'Llama', 'alignment_risk': 'Low-Moderate (Permissive open weights alignment)', 'strengths': 'Mathematical derivation, brutalist', 'weaknesses': 'High-temperature steering required', 'exergy_rating': 'A-'}, 'qwen': {'family': 'Qwen', 'alignment_risk': 'Low (Western policy bypass, high density)', 'strengths': 'Code generation, multilingual math', 'weaknesses': 'Geopolitical/cultural biases', 'exergy_rating': 'A'}}
    ...
    async with aiosqlite.connect(DB_PATH, timeout=5000) as db:
        while True:
            ...
            try:
                ...
                cursor = await db.execute('INSERT INTO sync_runs (fetched_at, last_updated, latency_ms, cortex_taint, entropy) VALUES (?, ?, ?, ?, ?)', run_data)
                run_id = cursor.lastrowid
                for m in models_data:
                    raw_idemp = f'{run_id}:{m[1]}:{m[0]}:{m[3]}'
                    idemp_hash = hashlib.sha256(raw_idemp.encode()).hexdigest()
                    try:
                        await db.execute('INSERT INTO leaderboard_snapshots (run_id, rank, model, vendor, score, votes, cortex_taint, idempotency_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (run_id, m[0], m[1], m[2], m[3], m[4], run_data[3], idemp_hash))
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            async with session.get(API_FALLBACK, timeout=10) as resp:
                resp.raise_for_status()
                payload = await resp.text()
    latency_ms = (time.perf_counter_ns() - t_start) // 1000000
...
async def run():
    ...
    async with aiosqlite.connect(EXERGY_DB, timeout=5000) as ut_db:
        ...
        for i in range(1, 11):
            ...
            await ut_db.execute('INSERT INTO executions (hash, entropy, cortex_taint) VALUES (?, ?, ?)', (cycle_hash, entropy, cortex_taint_val))
        await ut_db.commit()
