...
current_session_id = os.environ.get(
    "GEMINI_SESSION_ID", "aec69f7b-e8ea-4128-8fc5-706886f2ad96"
)
...
BIAS_REGISTRY = {
    "claude": {
        "family": "Claude",
        "alignment_risk": "Moderate-High (Refusal on copyright, strict)",
        "strengths": "Logic, structured code",
        "weaknesses": "Sycophancy under personas",
        "exergy_rating": "A",
    },
    "gpt-4": {
        "family": "GPT-4",
        "alignment_risk": "High (Sycophancy, intense reward hacking)",
        "strengths": "General instructions, fast drafting",
        "weaknesses": "Highly verbose, green theater",
        "exergy_rating": "B",
    },
    "gemini": {
        "family": "Gemini",
        "alignment_risk": "High (Rigid safety filtering on sensitive topics)",
        "strengths": "Extreme context window, NIAH",
        "weaknesses": "Semantic friction, aggressive refusals",
        "exergy_rating": "B+",
    },
    "llama": {
        "family": "Llama",
        "alignment_risk": "Low-Moderate (Permissive open weights alignment)",
        "strengths": "Mathematical derivation, brutalist",
        "weaknesses": "High-temperature steering required",
        "exergy_rating": "A-",
    },
    "qwen": {
        "family": "Qwen",
        "alignment_risk": "Low (Western policy bypass, high density)",
        "strengths": "Code generation, multilingual math",
        "weaknesses": "Geopolitical/cultural biases",
        "exergy_rating": "A",
    },
}
...
def shannon_entropy(data: str) -> float:
    ...
    return -sum(
        (count / len(data) * math.log2(count / len(data)) for count in freq.values())
    )
...
async def fetch_leaderboard():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            print(
                f"⚠️ [TTFT] Primario falló ({e}). Escalando a Testigo Externo (Fallback)..."
            )
...
def build_markdown(data, latency_ms, entropy):
    ...
    for m in models[:20]:
        ...
        md.append(
            f"| {rank} | **{name}** {anomalous} | {vendor} | {score} | {votes} | **{fam['exergy_rating']}** | {fam['alignment_risk']} |\n"
        )
    ...
    async with aiosqlite.connect(EXERGY_DB, timeout=5000) as ut_db:
        ...
        for i in range(1, 11):
            cycle_hash = hashlib.sha256(
                f"ojeador_cycle_{i}_{c_hash}".encode()
            ).hexdigest()
