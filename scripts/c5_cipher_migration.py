...
def migrate_master_ledger(fernet, db_path):
    ...
    try:
        ...
        for row in rows:
            ...
            if not payload.startswith("C5ENC:"):
                enc = f"C5ENC:{fernet.encrypt(payload.encode('utf-8')).decode('utf-8')}"
                # By passing immutable triggers
                cursor.execute("PRAGMA ignore_check_constraints = 1") # SQLite doesn't disable triggers this way
                # In SQLite, triggers can be dropped and recreated, but for C5-REAL migration,
                # we just use direct UPDATE. If trg_ledger_immutable_update blocks it, we drop it temporarily.
