...
def migrate_nexus_cache(fernet):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT count(name) FROM sqlite_master WHERE type='table' AND name='L3_inference_cache'"
        )
        ...
        for row in rows:
            ...
            if not payload.startswith("C5ENC:"):
                ...
                cursor.execute(
                    "UPDATE L3_inference_cache SET trace_payload = ? WHERE query_hash = ?",
                    (enc, row["query_hash"]),
                )
                updates += 1
        conn.commit()
...
def migrate_master_ledger(fernet, db_path):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT count(name) FROM sqlite_master WHERE type='table' AND name='ledger_entries'"
        )
        if cursor.fetchone()[0] == 0:
        ...
        for row in rows:
            ...
            if not payload.startswith("C5ENC:"):
                ...
                cursor.execute(
                    "UPDATE ledger_entries SET payload_json = ? WHERE seq = ?",
                    (enc, row["seq"]),
                )
                cursor.execute(
