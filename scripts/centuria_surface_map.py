...
def extract_generic_primitives(content: str):
    ...
    functions += re.findall('(?:const|let|var)\\s+([a-zA-Z0-9_]+)\\s*=\\s*(?:function|\\()', content)
    imports = re.findall('(?:import|require)\\s*\\(?[\'\\"]([^\'\\"]+)[\'\\"]', content)
...
def analyze_file(filepath: Path):
    ...
    return {'file_path': str(filepath), 'extension': filepath.suffix, 'entropy': entropy, 'lines': lines, 'classes': json.dumps(classes), 'functions': json.dumps(functions), 'imports': json.dumps(imports)}
...
def worker_thread(filepath: Path):
    ...
    conn.execute('PRAGMA busy_timeout=5000')
    try:
        conn.execute('\n            INSERT OR REPLACE INTO surface_map \n            (file_path, extension, entropy, lines, classes, functions, imports)\n            VALUES (?, ?, ?, ?, ?, ?, ?)\n        ', (data['file_path'], data['extension'], float(data['entropy']), data['lines'], data['classes'], data['functions'], data['imports']))
        conn.commit()
