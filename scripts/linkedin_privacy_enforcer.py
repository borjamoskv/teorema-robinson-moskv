...
sys.path.append(
    "$CORTEX_ROOT/.gemini/config/skills/Darwin_CGEvent_Injector/scripts"
)
try:
...
def enforce_matrix_node(node_id: str, data: dict, maestro: UIMaestro) -> Dict[str, Any]:
    ...
    try:
        ...
        for target in targets:
            ...
            for attempt in range(20):
                try:
                    ...
                    if res.status == "success":
                        print(
                            f"[SUCCESS] {node_id} Clicked '{target}' -> {res.latency_ms:.2f}ms"
                        )
                        res_list.append(
                            {"target": target, "latency_ms": res.latency_ms}
                        )
                        found = True
                        break
                except RuntimeError:
            ...
            if not found:
                raise Exception(
                    f"Thermodynamic Timeout (10s) waiting for element '{target}'"
                )
...
def main():
    ...
    if not os.path.exists(matrix_path):
        matrix_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "privacy_matrix.yaml"
        )
