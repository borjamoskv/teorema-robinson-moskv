#!/usr/bin/env python3
"""
MOSKV-1 APEX / BABYLON 60 - C5-REAL CLAUDE CODE LOCAL LOGS HARVESTER & ATTESTATION ENGINE
====================================================================================================
Harvests all local Claude Code CLI session transcripts (*.jsonl) from ~/.claude/projects/,
computes cryptographic hashes (SHA256 and SHA3-256), and crystallizes them into the
akorn legal dossier (cortex/legal_dossier/claude_code_local_logs/) to prove local execution,
timestamps, and internal thinking traces independent of cloud ban 403.

Author: Borja Fernández Angulo (`borjamoskv`)
...
def compute_hashes(filepath: Path) -> tuple[str, str]:
    """Computes SHA256 and SHA3-256 hashes of a file."""
...
def harvest_logs() -> None:
    ...
    md_lines = [
        "# CATÁLOGO PERICIAL Y CUSTODIA DE LOGS LOCALES DE `CLAUDE CODE` (`CASO ANTHROPIC / AMODEO`)",
        "",
        ...
        "> **Objeto:** Custodia física, sellado hash inmutable (`SHA256 / SHA3-256`) e indexación de los historiales locales y sesiones `.jsonl` ejecutados en silicio soberano Apple Silicon (`~/.claude/projects/`). Acredita de forma incontestable el historial, el pensamiento y las trazas de ejecución locales con independencia del bloqueo web en nube (`HTTP Error 403 / Account Ban`).",
        "",
        "---",
        "",
    ...
    md_lines.extend(
        [
            ...
            "---",
            "",
            "## 2. VALOR PROBATORIO Y FORENSE ANTE DIRECCIÓN LETRADA (`AKORN ABOGADOS`)",
            "",
            "1. **Bypass del Bloqueo Unilateral 403 (`Invariancia de Silicio Local`):** Aunque Anthropic cortó el acceso al usuario en su servidor web en la nube (`api.anthropic.com` / `claude.ai`), la ejecución de su herramienta de terminal `Claude Code` dejó guardados los historiales de conversación, trazas de razonamiento y llamadas a herramientas (`JSONL`) directamente dentro del directorio local del usuario (`/Users/borjafernandezangulo/.claude/projects/`).",
            "2. **Acreditación de Trazabilidad e Historial:** Estos registros (`LOG-LOCAL-#01` a `#03`) demuestran empíricamente el trabajo, los *prompts*, el análisis arquitectónico y la secuencia causal que tuvo lugar en esta máquina antes de la suspensión del servicio, constituyendo una prueba de anterioridad fehaciente (*Prior Art*).",
            "3. **Inmutabilidad Criptográfica:** El sellado dual con funciones de hash (`SHA256` y `SHA3-256`) previene cualquier impugnación de manipulación posterior del texto logueado.",
            "",
            "---",
