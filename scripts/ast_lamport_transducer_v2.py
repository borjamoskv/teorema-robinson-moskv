...
def refactor(filename, class_name):
    ...
    process_match = re.search('cur = await db\\.execute\\("SELECT lamport_t.*?raise RuntimeError\\("Insertion failed: event_id not persisted and not found"\\)', content, re.DOTALL)
    if process_match:
