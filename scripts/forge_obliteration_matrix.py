...
MATRIX_PATH = Path("/Users/borjafernandezangulo/30_BABYLON-60/scripts/300_primitivas_obliterar.yaml")

DOMAINS = {
    "UNIX_FS": ("OBLITERATE_FS_", "Aniquilación física de inodos y descriptores de archivo estancados."),
    "MEMORY_PID": ("OBLITERATE_MEM_", "Ejecución de señales SIGKILL y purgas de SRAM/Swap."),
    "NETWORK_SOCKETS": ("OBLITERATE_NET_", "Destrucción de sockets huérfanos y buffers TCP colapsados."),
    "DOCKER_OCI": ("OBLITERATE_OCI_", "Erradicación de contenedores zombies y volúmenes colgantes."),
    "GIT_VCS": ("OBLITERATE_VCS_", "Poda agresiva del árbol Merkle y recolección de basura (GC)."),
    "PYTHON_VENV": ("OBLITERATE_PY_", "Purga de bytecode __pycache__ y entornos virtuales corrompidos."),
    "NODE_V8": ("OBLITERATE_V8_", "Aniquilación de agujeros negros de dependencias (node_modules)."),
    "DATABASES_SQL": ("OBLITERATE_SQL_", "VACUUM BFT y truncado físico de tablas (Truncate/Drop)."),
    "MACOS_DARWIN": ("OBLITERATE_OSX_", "Liberación de presión Jetsam y purgas de kernel."),
    "LLM_TENSORS": ("OBLITERATE_TENSOR_", "Descarga de pesos de red neuronal inactivos de la VRAM/SRAM.")
}
...
def generate_300_primitives():
    ...
    for domain, (prefix, desc) in DOMAINS.items():
        ...
        for i in range(1, 31):
            ...
            # Add specific realistic commands for the first few in each domain to prove C5-REAL execution
