...
def main():
    ...
    try:
        ...
        return 1
    match = re.search("Ledger-Head:\\s+([a-f0-9]{64})", commit_msg)
    if not match:
        print(
            "[*] No Ledger-Head trailer found in the latest commit. Assuming non-bft commit."
        )
        return 0
    ...
    try:
        ...
        if git_head != db_head:
            print(f"\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m Divergence detected!")
