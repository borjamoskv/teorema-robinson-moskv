...
def worker_task(worker_id, num_inserts, db_path):
    ...
    while rows_inserted < num_inserts:
        ...
        for i in range(chunk_size):
            h = hashlib.md5(f'{worker_id}-{rows_inserted + i}-{time.time()}'.encode()).hexdigest()
            chunk.append((worker_id, h, time.time()))
        try:
            with conn:
                conn.executemany('INSERT INTO stress_1m (worker_id, entropy_hash, timestamp_utc) VALUES (?, ?, ?)', chunk)
...
def run_stress_test():
    db_path = '$CORTEX_ROOT/30_BABYLON-60/cortex/vault/stress_1M_ledger.db'
    ...
    print(f'[C5-REAL] INICIANDO 1,000,000 PRUEBAS DE ESTRÉS (WAL Concurrency N={num_workers})')
    t0 = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(worker_task, i, inserts_per_worker, db_path) for i in range(num_workers)]
        for idx, f in enumerate(concurrent.futures.as_completed(futures)):
            t_worker = f.result()
            print(f'  -> Worker {idx} completó {inserts_per_worker} inserciones en {t_worker:.2f}s')
    t1 = time.time()
    total_time = t1 - t0
    conn = sqlite3.connect(db_path)
