...
class SexagesimalCoordinate:
    ...
    def __init__(self, units: int, sixtieths: int, ticks: int):
        if not (0 <= sixtieths < 60 and 0 <= ticks < 60):
            raise ValueError(
                "[SIGKILL_State_Purge] Out of bounds for Base-60 coordinate."
            )
        self.units = units
...
class PhysicalMembraneState:
    ...
    def __init__(self, state_type: str, payload_hash: str, lamport_clock: int):
        ...
        if state_type not in valid_states:
            raise TypeError(
                f"[SIGKILL_State_Purge] Illegal state unrepresentable: {state_type}"
            )
        if state_type == "C4_Simulated_Buffer":
            raise RuntimeError(
                "[SIGKILL_State_Purge] C4-SIM state rejected by C5-REAL physical membrane during SHIP."
            )
...
class BFTMasterLedgerWAL:
    ...
    def append_c5_transaction(
        self, claim_payload: str, lamport_clock: int, agent_id: str
    ) -> str:
        with sqlite3.connect(self.db_path, timeout=5.0) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT taint_hash FROM master_ledger ORDER BY sequence_id DESC LIMIT 1;"
            )
            row = cursor.fetchone()
            ...
            raw_taint = (
                f"{prev_hash}||{claim_payload}||{lamport_clock}||{agent_id}".encode(
                    "utf-8"
                )
            )
            ...
            try:
                ...
                conn.rollback()
                raise RuntimeError(
                    f"[SIGKILL_State_Purge] BFT/WAL integrity violation: {e}"
                )
    ...
    def verify_ledger_integrity(self) -> bool:
        with sqlite3.connect(self.db_path, timeout=5.0) as conn:
            ...
            for prev_h, payload, clock, agent, taint_h in rows:
                ...
                if taint_h != calc_t:
                    raise AssertionError(
                        f"[SIGKILL_State_Purge] Taint hash mismatch! Expected {calc_t}, got {taint_h}"
                    )
...
def run_c5_verification_suite():
    ...
    assert div_coord.to_string() == "0:50:00_BASE60", (
        f"Unexpected coord: {div_coord.to_string()}"
    )
    ...
    print(
        "[✓] PRIMITIVA-LOGOS-002: F# Algebraic Membrane / Illegal State rejection verified."
    )
    ...
    t1 = ledger.append_c5_transaction(
        "CLAIM: LOGOS_BASE60_COLLAPSED", 101, "borjamoskv"
    )
