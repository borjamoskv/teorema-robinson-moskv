...
process_match = re.search('cur = await db\\.execute\\("SELECT lamport_t.*?db_row = await cursor\\.fetchone\\(\\)', la, re.DOTALL)
if process_match:
...
process_match_ml = re.search('cur = await db\\.execute\\("SELECT lamport_t.*?db_row = await cursor\\.fetchone\\(\\)', ml, re.DOTALL)
if process_match_ml:
