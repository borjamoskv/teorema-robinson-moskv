...
class CIBMasterLedgerDaemon:
    ...
    async def _physical_writer_loop(self):
        ...
        try:
            with sqlite3.connect(
                self.db_path, timeout=5.0, isolation_level="IMMEDIATE"
            ) as conn:
                while True:
                    ...
                    if action_type == "INGEST":
                        ...
                        continue
                    cur.execute(
                        "SELECT current_hash FROM master_ledger ORDER BY id DESC LIMIT 1"
                    )
                    row = cur.fetchone()
                    prev_hash = row[0] if row else "GENESIS"
                    current_hash = hashlib.sha256(
                        f"{prev_hash}{log_action}{cortex_taint}".encode()
                    ).hexdigest()
                    cur.execute(
    ...
    async def calculate_roi_dissonance(
        ...
        await self.write_queue.put(
            {"action_type": "ROI_CALC", "payload": {"log": log_msg}}
        )
        return json.dumps(
            {
                "node_id": node_id,
