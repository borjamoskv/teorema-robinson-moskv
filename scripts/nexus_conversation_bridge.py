...
def sync_transcripts(
    conn: sqlite3.Connection, force: bool = False, purge: bool = False
):
    with console.status(
        "[bold cyan]Ingestando matriz conversacional en SQLite FTS5...[/bold cyan]"
    ):
        ...
        for transcript_file in files:
            ...
            try:
                ...
                if not force:
                    cursor.execute(
                        "SELECT last_modified FROM sync_metadata WHERE conversation_id = ?",
                        (conv_id,),
                    )
                    row = cursor.fetchone()
                    if row and row[0] >= mtime:
                        ...
                        continue
                cursor.execute(
                    "DELETE FROM transcripts_fts WHERE conversation_id = ?", (conv_id,)
                )
                cursor.execute(
                    "DELETE FROM merkle_ledger WHERE conversation_id = ?", (conv_id,)
                )
                prev_hash = "0" * 64
                with open(transcript_file, "r", encoding="utf-8") as f:
                    for idx, line in enumerate(f):
                        ...
                        try:
                            ...
                            console.print(
                                f"[bold red]ANERGÍA DETECTADA (Corrupted JSONL):[/bold red] {e} in {transcript_file}"
                            )
                            continue
                        ...
                        if content:
                            ...
                            if len(content) > 100000:
                                continue
                            raw_payload = f"{prev_hash}{conv_id}{step_idx}{source}{content}".encode(
                                "utf-8"
                            )
                ...
                if purge:
                    try:
                        ...
                        console.print(
                            f"[bold red]💥 PURGED ENTROPY:[/bold red] {transcript_file}"
                        )
                    except OSError as e:
                        console.print(
                            f"[bold red]Error purging {transcript_file}: {e}[/bold red]"
                        )
            except FileNotFoundError:
                continue
        console.print(
            f"[bold green]✔ SYNC COMPLETE[/bold green] [cyan]{synced}[/cyan] conversiones mutadas, [dim]{skipped}[/dim] omitidas."
        )


def search_fts(
    conn: sqlite3.Connection, query: str, limit: int, context_window: int = 0
):
    time.time()
    import re
    ...
    console.print(
        Panel(
            f"Rastreando entropía: [bold yellow]'{safe_query}'[/bold yellow] (Límite: {limit}) | Contexto ±{context_window}",
            title="NEXUS FTS5 BRUTALIST",
            border_style="cyan",
        )
    )
    ...
    for i, (conv_id, step_idx, source, c_hash, snip) in enumerate(rows):
        ...
        clean_snip = clean_snip.replace("[bold yellow]", "[bold red]").replace(
            "[/bold yellow]", "[/bold red]"
        )
        merkle_str = f" | [magenta]Merkle: {c_hash[:8]}[/magenta]" if c_hash else ""
        console.print(
            Panel(
                f"[bold yellow]MATCH {i + 1}[/bold yellow] | Conv: [dim]{conv_id}[/dim] | Step: {step_idx} | Source: [cyan]{source}[/cyan]{merkle_str}\n{clean_snip}",
        ...
        if context_window > 0:
            ...
            for c_idx, c_source, c_content, c_hash in ctx_rows:
                ...
                urls = re.findall(
                    "(https?://[^\\s]+|file://[^\\s]+|/[a-zA-Z0-9_/-]+\\.[a-zA-Z0-9]+)",
                    raw_content,
                )
                taint_str = f" | 🔗 {len(urls)} refs" if urls else ""
                sentinel_hashes = re.findall(
                    "\\[[a-zA-Z0-9_/-]+\\s([a-f0-9]{7,40})\\]|([a-f0-9]{7,40})\\s(?:feat|fix|refactor|docs|chore|test)",
                    raw_content,
                )
                sentinel_str = ""
                for group in sentinel_hashes:
                    ...
                    if hash_val:
                        sentinel_str += (
                            f" | [bold green]🛡️ SENTINEL {hash_val[:7]}[/bold green]"
                        )
                merkle_str = (
                    f" | [magenta]Merkle: {c_hash[:8]}[/magenta]" if c_hash else ""
                )
                console.print(
                    Panel(
                        renderable,
                        title=f"[{style}]{prefix} Step: {c_idx} | Source: {c_source}{taint_str}{merkle_str}[/{style}]{sentinel_str}",
                        border_style=border,
                        padding=(0, 2),
                    )
                )
    ...
    console.print(
        f"[bold green]🛡️ SAGA PIPELINE[/bold green] Integridad de Libro Mayor validada: {merkle_count} bloques anclados."
    )
...
def uds_server_thread():
    ...
    while True:
        ...
        try:
            ...
            client.sendall(
                json.dumps(
                    {"error": "SAGA_ABORT: IPC Read Timeout (Anergía Evadida)"}
                ).encode("utf-8")
            )
            client.close()
            continue
        ...
        if payload_str:
            try:
                ...
                if "query" not in payload:
                    raise ValueError(
                        "SAGA-1_VIOLATION: Payload inválido, falta 'query'."
                    )
                q = payload["query"]
                limit = payload.get("limit", 10)
                if not isinstance(q, str) or not isinstance(limit, int):
                    raise ValueError(
                        "SAGA-3_VIOLATION: Tipado estricto roto. Se requiere 'query' [str] y 'limit' [int]."
                    )
                import hashlib
                ...
                response = json.dumps(
                    {"status": "ok", "cortex_taint": taint_hash, "results": results}
                )
                client.sendall(response.encode("utf-8"))
            except json.JSONDecodeError as e:
                client.sendall(
                    json.dumps({"error": f"SAGA_ABORT (JSON): {str(e)}"}).encode(
                        "utf-8"
                    )
                )
            except ValueError as e:
                client.sendall(
                    json.dumps({"error": f"SAGA_ABORT (VALUE): {str(e)}"}).encode(
                        "utf-8"
                    )
                )
        client.close()
...
def start_daemon(conn: sqlite3.Connection):
    try:
        ...
        console.print(
            "[bold red]FATAL:[/bold red] 'watchdog' no instalado. Usa: pip install watchdog"
        )
        sys.exit(1)
    ...
    console.print(
        f"[bold magenta]👁️ NEXUS WATCHER[/bold magenta] Daemon iniciado sobre {BRAIN_DIR}. Presiona Ctrl+C para abortar."
    )
    try:
        while True:
            time.sleep(1)
...
def main():
    ...
    parser.add_argument(
        "--sync",
        action="store_true",
        help="Forzar sincronización delta de logs a SQLite.",
    )
    parser.add_argument(
        "--force-sync",
        action="store_true",
        help="Forzar purga y resincronización total.",
    )
    parser.add_argument(
        "--purge",
        action="store_true",
        help="Elimina los jsonl tras la ingesta para reducir entropía (C5-REAL).",
    )
    parser.add_argument(
        "--context",
        type=int,
        default=0,
        help="Extrae N pasos anteriores y posteriores a la inyección (Contexto Causal).",
    )
    parser.add_argument(
        "--daemon",
        action="store_true",
        help="Inicia un Watcher en segundo plano para ingesta O(1).",
    )
