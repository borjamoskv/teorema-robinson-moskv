...
class DolaOpenAIWrapper:
    ...
    def __init__(self, api_key='dummy_key_32_chars_aaaaaaaaaaaaaa'):
        self.client = OpenAI(base_url='https://dola.us.openbytealfa.com/v1', api_key=api_key)
    ...
    def chat_completion(self, messages, temperature=0.7, **kwargs):
        ...
        return self.client.chat.completions.create(model=self.model, messages=messages, temperature=temperature, extra_body=extra_body, **kwargs)
if __name__ == '__main__':
    wrapper = DolaOpenAIWrapper()
    print('Initiating 3ms TTFT bypass via OpenAI SDK...')
    response = wrapper.chat_completion(messages=[{'role': 'user', 'content': '¿Cuál es la latencia estructural de este nodo?'}])
