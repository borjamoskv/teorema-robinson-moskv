...
def deprecate_and_purge(total_loc, anergic_files):
    if total_loc > LOC_THRESHOLD:
        print(f'\x1b[1;31m[SIGKILL_STATE_PURGE]\x1b[0m Codebase size {total_loc} > {LOC_THRESHOLD}. Initiating Apoptosis.')
    ...
    for file, ent in anergic_files:
        print(f'\x1b[1;33m[WARNING]\x1b[0m Anergic AST detected in: {file} (Entropy: {ent:.4f} bits/char)')

def init_ontology():
    os.makedirs(os.path.dirname(ONTOLOGY_YAML), exist_ok=True)
    if not os.path.exists(ONTOLOGY_YAML):
        default_ontology = {'entities': [{'id': 'ENT-001', 'name': 'BFT_STATE_LOOP', 'type': 'invariant'}, {'id': 'ENT-002', 'name': 'ZERO-ANERGY', 'type': 'primitive'}, {'id': 'ENT-003', 'name': 'C5-REAL', 'type': 'level'}]}
...
if __name__ == '__main__':
    ...
    print('\x1b[1;32m[ENTROPY_SWEEPER_DAEMON]\x1b[0m Scan Complete.')
