...
for a, t, b, i in itertools.product(D_A, TRANS, D_B, INV):
    ...
    primitives[f'ISO_STRUCT_{idx:03d}'] = {'Claim': f'Isomorfismo Estructural: {a} <-> {b}', 'Proof': {'Base': f'hash::{base_hash}', 'Range': '[0, 1]', 'Confidence': 'C5-REAL', 'Transducer': t, 'Invariant': i, 'Mapping': f'f: {a} -> {b} vía {t} bajo {i}'}}
    idx += 1
