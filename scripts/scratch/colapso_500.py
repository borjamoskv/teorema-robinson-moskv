...
def extract_entropy():
    ...
    for match in matches:
        ...
        domain = 'UNKNOWN'
        if 1 <= uid <= 25:
            domain = 'Sustrato_Tipado'
        elif 26 <= uid <= 50:
            domain = 'Relaciones_Incidencia'
        elif 51 <= uid <= 75:
            domain = 'Operaciones_Algebraicas'
        elif 76 <= uid <= 100:
            domain = 'Orden_Geometria_Topologia'
        elif 101 <= uid <= 125:
            domain = 'Descomposicion_Interfaces'
        elif 126 <= uid <= 150:
            domain = 'Dinamica_Concurrencia'
        elif 151 <= uid <= 175:
            domain = 'Logica_Restricciones'
        elif 176 <= uid <= 200:
            domain = 'Correspondencia_Sintesis'
        elif 201 <= uid <= 225:
            domain = 'Magnitudes_Pesos'
        elif 226 <= uid <= 250:
            domain = 'Canonizacion_Verificacion'
        elif 251 <= uid <= 275:
            domain = 'Signatura_Cardinalidad'
        elif 276 <= uid <= 300:
            domain = 'Incidencia_Matrices_Refinamiento'
        elif 301 <= uid <= 325:
            domain = 'Conectividad_Caminos_Ciclos'
        elif 326 <= uid <= 350:
            domain = 'Invariantes_Algebraicos'
        elif 351 <= uid <= 375:
            domain = 'Ordenes_Reticulos'
        elif 376 <= uid <= 400:
            domain = 'Simetria_Descomposicion'
        elif 401 <= uid <= 425:
            domain = 'Dinamica_Lenguajes_Concurrencia'
        elif 426 <= uid <= 450:
            domain = 'Topologia_Homologia'
        elif 451 <= uid <= 475:
            domain = 'Pesos_Medidas_Espectros'
        elif 476 <= uid <= 500:
            domain = 'Logica_Conteos_Completitud'
        sig = f'{uid}|{node_type}|{domain}|{name}|{desc}'
        causal_hash = hashlib.blake2s(sig.encode()).hexdigest()
        try:
            cursor.execute('INSERT INTO structural_primitives (id, domain, name, description, type, causal_hash) VALUES (?, ?, ?, ?, ?, ?)', (uid, domain, name, desc, node_type, causal_hash))
            count += 1
