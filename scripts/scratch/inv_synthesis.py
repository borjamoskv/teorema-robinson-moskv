...
for sys, fault, verifier, halt in itertools.product(SYSTEMS, FAULTS, VERIFIERS, HALTS):
    ...
    check_invariants[f'INV_CHECK_{idx:03d}'] = {'Claim': f'Verificacion estricta de {sys} ante {fault}', 'Proof': {'Base': f'hash::{hash_proof}', 'Range': '[0, 1]', 'Confidence': 'C5-REAL', 'Target_System': sys, 'Fault_Vector': fault, 'Verification_Operator': verifier, 'Halting_Condition': halt, 'Logic': f'If {verifier}({sys}({fault})) fails, trigger {halt}'}}
    idx += 1
out_path = '$CORTEX_ROOT/30_BABYLON-60/cortex/ontology/inv_check_500.yaml'
