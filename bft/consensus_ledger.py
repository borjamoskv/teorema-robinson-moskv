...
class BFT_Ledger:
    ...
    def invoke_subagent(self, mutation: StateMutation, f: int, swarm_signatures: Dict[str, str]) -> bool:
        ...
        self.conn.execute('INSERT INTO state_log (mutation_hash, agent_id, payload, ts) VALUES (?, ?, ?, ?)', (mutation_hash, mutation.agent_id, json.dumps(mutation.payload, separators=(',', ':'), sort_keys=True, ensure_ascii=False), mutation.timestamp))
        return True
