...
class BFTLedgerActor:
    def __init__(self, db_path: Path) -> None:
        ...
        self._queue: asyncio.Queue[
            tuple[LedgerEvent, asyncio.Future[Dict[str, Any]]]
        ] = asyncio.Queue()
        ...
        async with aiosqlite.connect(self._db_path) as db:
            ...
            for row in rows:
                ...
                if vault_key and payload_json.startswith("C5ENC:"):
                    ...
                    payload_json = fernet.decrypt(
                        payload_json[6:].encode("utf-8")
                    ).decode("utf-8")
    ...
    async def _process(
        ...
        if not event.cortex_taint or not isinstance(event.cortex_taint, str):
            raise ValueError(
                "INV_BFT_03: cortex_taint must be a non-empty string representing the causal trace"
            )
        ...
        try:
            ...
            cursor = await db.execute(
                "SELECT seq, entry_hash FROM ledger_entries WHERE event_id = ?",
                (event_id,),
            )
            ...
            if row:
                ...
                future.set_result(
                    {"seq": row[0], "event_id": event_id, "entry_hash": row[1]}
                )
                return
            vault_key = os.environ.get("CORTEX_VAULT_KEY")
            if vault_key:
                fernet = Fernet(vault_key.encode("utf-8"))
                stored_payload = fernet.encrypt(payload_json.encode("utf-8")).decode(
                    "utf-8"
                )
            ...
            if db_row is None:
                ...
                if db_row is None:
                    raise RuntimeError(
                        "Insertion failed: event_id not persisted and not found"
                    )
            await db.execute("COMMIT")
            future.set_result(
                {"seq": db_row[0], "event_id": event_id, "entry_hash": db_row[1]}
            )
            ...
            try:
                ...
                future.set_exception(exc)
                raise RuntimeError(
                    "Cascading Rollback Defense triggered: connection aborted during IntegrityError rollback"
                ) from rollback_exc
            future.set_exception(exc)
            ...
            try:
                ...
                future.set_exception(exc)
                raise RuntimeError(
                    "Cascading Rollback Defense triggered: connection aborted"
                ) from rollback_exc
            future.set_exception(exc)
