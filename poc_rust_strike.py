...
# Import the newly compiled C5-REAL PyO3 extension
try:
    import strike_rs
except ImportError:
    print("\033[1;31m[CORTEX APOPTOSIS]\033[0m strike_rs no compilado o instalado en el venv.")
    exit(1)
...
def compute_kolmogorov_python(text_data: str) -> dict:
    ...
    return {
        ...
        "raw_size": raw_size,
        "compression_ratio": (Decimal(raw_size) / Decimal(compressed_size)) if compressed_size > 0 else Decimal("1.0"),
    }
...
def main():
    ...
    # Generate 5MB of somewhat random stochastic data (high entropy)
