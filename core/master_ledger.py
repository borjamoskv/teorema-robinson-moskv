...
class BFTLedgerActor:
    ...
    def append(self, event: LedgerEvent) -> asyncio.Future[Dict[str, Any]]:
        if self._task and self._task.done():
            raise RuntimeError(
                "C5 BFT Fail-Fast: Zombie Actor detected. The _worker task has died in silence."
            )
    ...
    async def _process(
        self, db: aiosqlite.Connection, event: LedgerEvent, future: asyncio.Future
    ) -> None:
        ...
        try:
            ...
            try:
                ...
                await db.close()
                raise RuntimeError(
                    f"C5 BFT Fail-Fast: ROLLBACK failed, closing irrecoverable connection. Root error: {exc}. Rollback error: {rollback_exc}"
                ) from exc
            if not future.done():
                future.set_exception(exc)
