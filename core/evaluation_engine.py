...
class EvaluatorT2:
    ...
    def calculate_utility(
        self, quality_bp: int, execution_receipt: dict, privacy_violation: bool = False
    ) -> float:
        ...
        U_e = (
            self.LAMBDA_Q * float(quality_bp)
            + self.LAMBDA_L * L_e
            + self.LAMBDA_C * C_e
        )
    ...
    def compute_regret(
        ...
        shadow_executions: List[dict],
        quality_scores: Dict[str, int],
    ) -> dict:
        primary_quality = quality_scores.get(primary_execution["receipt_id"], 0)
        u_primary = self.calculate_utility(
            primary_quality, primary_execution["payload"]
        )
        propensities = {
