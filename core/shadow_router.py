...
class ShadowRouter:
    ...
    async def route_request(self, prompt: str, context: dict) -> Tuple[Dict, Dict]:
        ...
        decision_receipt = {'schema': 'proof-of-route/decision/v0.2', 'receipt_id': f'dr_{secrets.token_hex(8)}', 'issued_at': '2026-07-10T14:32:08.442Z', 'payload': decision_payload, 'payload_hash': decision_hash, 'signature': {'algorithm': 'Ed25519', 'key_id': 'did:key:z6Mkmock', 'value': self.signer.sign(decision_hash)}}
        ...
        execution_receipt = {'schema': 'proof-of-route/execution/v0.2', 'receipt_id': f'er_{secrets.token_hex(8)}', 'decision_receipt_hash': decision_hash, 'payload': execution_payload, 'payload_hash': exec_hash, 'signature': {'algorithm': 'Ed25519', 'key_id': 'did:key:z6Mkmock', 'value': self.signer.sign(exec_hash)}}
