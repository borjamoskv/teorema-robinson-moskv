...
class ShadowRouter:
    ...
    async def _execute_route(self, model_id: str, prompt: str) -> Dict[str, Any]:
        ...
        return {'status': 'success', 'ttft_ms': ttft_ms, 'total_latency_ms': total_latency_ms, 'input_tokens': 100, 'output_tokens': 50, 'cost_microusd': 4200, 'fallback_used': False, 'response_commitment': f'hmac-sha256:{secrets.token_hex(32)}', 'provider_receipt_hash': f'sha256:{secrets.token_hex(32)}'}
    ...
    async def route_request(self, prompt: str, context: dict) -> Tuple[Dict, Dict]:
        ...
        decision_payload = {'request_commitment': f'hmac-sha256:{secrets.token_hex(32)}', 'policy_id': 'arcstride-v4.2.1', 'policy_hash': f'sha256:{secrets.token_hex(32)}', 'candidate_set_hash': f'sha256:{secrets.token_hex(32)}', 'selected_route': {'provider': 'provider-x', 'model_alias': 'gemini-2.0-flash', 'model_version': 'gemini-2.0-flash-2026-07-01', 'region': 'eu-west'}, 'predictions': {'quality_lcb_basis_points': 8400, 'ttft_p95_ms': 3400, 'expected_cost_microusd': 4200, 'failure_probability_basis_points': 60}, 'selection_propensity_basis_points': 9200, 'route_confidence_basis_points': 8300, 'shadow_eligible': self._check_shadow_eligibility(context)}
        decision_hash = self._jcs_hash(decision_payload)
        decision_receipt = {'schema': 'proof-of-route/decision/v0.2', 'receipt_id': f'dr_{secrets.token_hex(8)}', 'issued_at': '2026-07-10T14:32:08.442Z', 'payload': decision_payload, 'payload_hash': decision_hash, 'signature': {'algorithm': 'Ed25519', 'key_id': 'did:key:z6Mkmock', 'value': self.signer.sign(decision_hash) if self.signer else ''}}
        ...
        execution_receipt = {'schema': 'proof-of-route/execution/v0.2', 'receipt_id': f'er_{secrets.token_hex(8)}', 'decision_receipt_hash': decision_hash, 'payload': execution_payload, 'payload_hash': exec_hash, 'signature': {'algorithm': 'Ed25519', 'key_id': 'did:key:z6Mkmock', 'value': self.signer.sign(exec_hash) if self.signer else ''}}
