...
def _verify_attestation(node_id: str, payload_digest: str, attestation: str, key: bytes) -> bool:
...
class ValidatorNode:
    ...
    def __init_subclass__(cls, **kw):
        ...
        if 'is_mock' in cls.__dict__:
            raise TypeError('Prohibido is_mock en nodos validadores [STUB-BFT-STRICT-001]')
...
def _merkle_commit(payload_digest: str, attestations: list[str], log_path: Path) -> str:
    ...
    entry = {'ts': time.strftime('%Y-%m-%dT%H:%M:%S%z'), 'payload_digest': payload_digest, 'attestations': attestations, 'prev_root': prev_root, 'root': root}
    with open(log_path, 'a', encoding='utf-8') as fh:
        fh.write(json.dumps(entry, separators=(',', ':')) + '\n')
    return root

def _reject(reason: str, quorum: int, attestations: list[str]) -> ConsensusResult:
    print(f'REJECT [STUB-BFT-STRICT-001]: {reason} (quorum={quorum}/{QUORUM})', file=sys.stderr)
    return ConsensusResult(False, quorum, attestations, reason)

def validate_mutation(payload: MutationPayload, merkle_log: str | Path=DEFAULT_MERKLE_LOG) -> ConsensusResult:
...
if __name__ == '__main__':
    src = b'x = 1\n'
    p = MutationPayload(diff=src, author='borja', timestamp=time.strftime('%Y-%m-%dT%H:%M:%S%z'), declared_digest=sha256_bytes(src), falsification=lambda: True)
    r = validate_mutation(p)
    print(f'STUB-BFT-STRICT-001: accepted={r.accepted} quorum={r.quorum}/{QUORUM} root={r.merkle_root[:16]}… [C5-REAL]')
