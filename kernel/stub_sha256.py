...
if "sha256" not in hashlib.algorithms_guaranteed:
    print(
        "CRITICAL HALT [STUB-SHA256-001]: sha256 no garantizado en hashlib.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def sha256_bytes(raw: bytes) -> str:
    if not isinstance(raw, bytes):
        raise TypeError(
            "STUB-SHA256-001 acepta bytes crudos; para str usar sha256_text (UTF-8 estricto)."
        )
...
def assert_match(raw: bytes, expected_digest: str) -> str:
    ...
    if actual != expected_digest.lower():
        print(
            f"CRITICAL HALT [STUB-SHA256-001]: integrity violation. expected={expected_digest.lower()} actual={actual}",
            file=sys.stderr,
        )
        sys.exit(1)
    return actual
