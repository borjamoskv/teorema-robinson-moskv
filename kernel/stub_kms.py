...
SCAN_TARGETS = (str(DEFAULT_KEY_DIR / '**'), str(Path.home() / '.ssh' / 'id_*'), '$CORTEX_ROOT/.env', '$CORTEX_ROOT/cortex/**/*.pem')
...
def enforce(file_path: str | Path) -> PermissionState:
    ...
    if corrected != REQUIRED_MODE:
        print(f'CRITICAL HALT [STUB-KMS-0600-001]: uncorrectable mode on {p} ({oct(corrected)})', file=sys.stderr)
        sys.exit(1)
    _append_audit(p, mode, corrected)
    print(f'ALERT [STUB-KMS-0600-001]: corrected {p} {oct(mode)} -> {oct(corrected)}', file=sys.stderr)
...
def store_key(name: str, material: bytes, key_dir: str | Path=DEFAULT_KEY_DIR) -> Path:
...
if __name__ == '__main__':
    ...
    print(f'STUB-KMS-0600-001 boot scan: {len(states)} files, {fixed} corrected [C5-REAL]')
