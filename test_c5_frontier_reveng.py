...
def test_ttft_heuristic_is_empirical() -> None:
    ...
    for model_target, signal_data, confidence in rows:
        assert confidence == "C5-REAL", (
            f"APOPTOSIS FAILED: Heurística detectada ({confidence}) en {model_target}."
        )
        assert "estimated" not in signal_data.lower(), (
            "APOPTOSIS FAILED: Teatro Verde 'estimated' detectado."
        )
        assert "scales linearly" not in signal_data.lower(), (
            "APOPTOSIS FAILED: Teatro Verde 'scales linearly' detectado."
        )
    conn.close()
