...
LEDGER_PATH = os.environ.get(
    "CORTEX_LEDGER_PATH", os.path.join(os.path.dirname(__file__), "nexus_anchors.db")
)
...
def init_ledger():
    ...
    with conn:
        ...
        if cols and "payload_hash" not in cols:
            conn.execute(
                "ALTER TABLE master_ledger ADD COLUMN payload_hash TEXT DEFAULT ''"
            )
        ...
        if cursor.fetchone()[0] == 0:
            ...
            conn.execute(
                ...
                (
                    0,
                    "UID0_MOSKV",
                    gen_payload_bytes.decode("utf-8"),
                    p_hash,
                    "0" * 64,
                    "CORTEX-TAINT:GENESIS",
                    e_hash,
                ),
            )
...
def compute_entry_hash(
    prev_hash: str, payload_bytes: bytes, agent_id: str, lamport_t: int, taint: str
) -> str:
    hasher = get_hasher()
...
def append_block(payload: dict, agent_id: str, taint_prefix: str) -> str:
    ...
    with conn:
        ...
        cursor.execute(
            "SELECT entry_hash FROM master_ledger WHERE payload_hash = ?",
            (payload_hash,),
        )
        ...
        cursor.execute(
            "SELECT lamport_t, entry_hash FROM master_ledger ORDER BY seq DESC LIMIT 1"
        )
        row = cursor.fetchone()
        if not row:
        ...
        new_hash = compute_entry_hash(
            prev_hash, payload_bytes, agent_id, new_lamport_t, taint
        )
...
def verify_ledger() -> bool:
    ...
    c = conn.cursor()
    c.execute(
        "SELECT lamport_t, agent_id, payload, prev_hash, cortex_taint, entry_hash FROM master_ledger ORDER BY lamport_t ASC"
    )
    rows = c.fetchall()
    for i in range(1, len(rows)):
        ...
        if (
            compute_entry_hash(curr[3], payload_bytes, curr[1], curr[0], curr[4])
            != curr[5]
        ):
