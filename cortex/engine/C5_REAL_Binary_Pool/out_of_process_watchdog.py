...
def monitor_llm_wal():
    print(
        "\033[38;2;176;38;255m[WATCHDOG]\033[0m Iniciando External Restart-Time Compensation. Observando WAL..."
    )
                ...
                if current_lamport > last_lamport:
                    ...
                    for m_hash, exit_code, entropy, prompt in mutations:
                        ...
                        if exit_code != 0 or entropy > 4.5:
                            ...
                            print(
                                f"-> Entropía anómala ({entropy}) o Exit Code ({exit_code})."
                            )
                            print(
                                "-> Ejecutando Compensación Externa (Rollback / Hard Restart)..."
                            )

                            # Compensación: Eliminación física de la mutación corrupta del Ledger
                            conn.execute(
                                "DELETE FROM ultrathink_ledger WHERE hash = ?",
                                (m_hash,),
                            )
                            conn.commit()
                            print(
                                "\033[38;2;180;230;176m[COMPENSATION SUCCESS]\033[0m Estado BFT restaurado. Anergía purgada."
                            )
