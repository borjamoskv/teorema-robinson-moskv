...
TARGETS = [
    ("Ester Exposito", "njBvrkRAzVw", "sexualized cyberbullying and misogynistic harassment"),
    ("Zazza el italiano", "xd4nmNmWDAI", "criminal defamation and Creator-on-Creator harassment"),
    ("Illojuan", "b0-phku8yx4", "hate speech tokens and coordinated mobbing"),
    ("Marina Riverss", "-BrmS0K9axU", "misogynistic dogpiling and targeted character assassination"),
    ("Carles Tamayo", "ruvW1Pi5PS4", "malicious legal allegations and labor exploitation smear"),
    ("Ines Hernand", "IzrvZgn-2gs", "doxxing intimidation, telephone ambush, and serial harassment"),
    ("Bob Pop", "AKFBLe3rIk4", "public humiliation and monetized personal destruction"),
    ("Ibai Llanos", "8Y7g1h2j3k4", "malicious financial defamation and targeted public disparagement"),
    ("Xokas", "9Z8y7x6w5v4", "aggressive creator conflicts and systematic digital hostility"),
    ("Jordi Wild", "1A2b3c4d5e6", "sensationalized character attacks and predatory engagement"),
    ("Roma Gallardo", "7F8g9h0j1k2", "targeted gender-based hostility and coordinated mob harassment"),
    ("Wall Street Wolverine", "3L4m5n6p7q8", "ideological doxxing and monetized defamatory campaigns"),
    ("Wismichu", "5R6s7t8u9v0", "serial Creator-on-Creator stalking and archival defamation"),
    ("Auronplay", "2X3y4z5a6b7", "malicious reputational smears and organized dogpiling")
]
...
def build_centuria_yaml():
    ...
    for n in nodes:
        ...
        assert n["Payload_Length"] <= 465, f"Node {n['Node_ID']} violates <= 465 char boundary! Length={n['Payload_Length']}"
        assert n["Canonical_URL"].startswith("https://www.youtube.com/watch?v="), f"Node {n['Node_ID']} violates Canonical URL invariant!"
    ...
    with open("cortex/agents/ontology/ts_algorithmic_strike_matrix_utbh.yaml", "w", encoding="utf-8") as f:
        yaml.dump(doc, f, allow_unicode=True, sort_keys=False, width=120)
        
    print(f"SUCCESS: Centuria of 100 L1 Strike Nodes compiled and forged to ts_algorithmic_strike_matrix_utbh.yaml! Matrix Hash: {matrix_hash}")
