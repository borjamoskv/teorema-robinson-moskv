...
def tokenize_ngrams(text, n=2):
    ...
    tokens = text.split()
    stopwords = {'el','la','los','las','un','una','unos','unas','y','e','o','u','de','del','a','en','que','para','por','con','sin','sobre','como','es','son','no','si','se','su','sus','al','lo'}
...
def purge_entity(filepath, entity_id):
    ...
    try:
        ...
        if removed > 0:
            fd, tmp_path = tempfile.mkstemp(dir=os.path.dirname(filepath), suffix=".macrofago")
            try:
...
def run_validation_gate():
    ...
    if not os.path.exists(gate):
        print("[-] GATE AUSENTE (validate_ontology.py). Fail-closed: apoptosis DENEGADA. Usa --no-gate para forzar.")
        return False
...
def _git(directory, *args):
    return subprocess.run(["git", "-C", directory] + list(args), check=True, capture_output=True, text=True)
...
def git_preanchor(directory, files):
    ...
    try:
        ...
        _git(directory, "commit", "-m", "[Macrófago] Anchor pre-apoptosis: estado previo asegurado", "--no-verify")
        print("[+] Git Sentinel: anchor pre-apoptosis commiteado (working tree estaba sucio).")
        return True
...
def main():
    parser = argparse.ArgumentParser(description="Macrófago Ontológico Ouroboros (C5-REAL v2.2)")
    parser.add_argument("--dir", type=str, default=".", help="Directorio objetivo")
    parser.add_argument("--execute", action="store_true", help="Ejecuta la apoptosis real en disco")
    parser.add_argument("--threshold", type=float, default=0.08, help="Umbral Jaccard (default 0.08)")
    parser.add_argument("--no-gate", action="store_true", help="Saltar el Validation Gateway (entropía asumida)")
    ...
    if is_dry_run:
        print("[!] EJECUCIÓN DRY-RUN: Usa --execute para mutar el disco y crear commits.")
    elif args.no_gate:
    ...
    for i in range(len(all_entities)):
        ...
        for j in range(i + 1, len(all_entities)):
            ...
            if ent_i['id'] == ent_j['id'] or sim >= threshold:
                ...
                redundancies.append({
                    'score': round(sim, 3),
                    'node_A': ent_i['id'], 'node_B': ent_j['id'],
                    'filepath_B': ent_j['file'],
                    'filename_A': ent_i['filename'], 'filename_B': ent_j['filename'],
    ...
    print(f"[+] Alerta de Anergía: Se detectaron {total_anergy_score} marcadores de 'Green Theater' en el sistema.")
    print(f"[*] Entidades marcadas para apoptosis: {sum(len(v) for v in purges_queue.values())}")
    ...
    with open(yaml_out, 'w', encoding='utf-8') as f:
        ...
        f.write("Proof: { Base: N-Gram Jaccard Index & Anergy Lexicon, Confidence: C5-REAL }\n")
    ...
    if is_dry_run:
        ...
        for filepath, ids in purges_queue.items():
            print(f"  [Presa] {os.path.basename(filepath)} -> IDs a purgar: {list(ids)}")
    ...
    for filepath, ids in purges_queue.items():
        ...
        if purged_in_file > 0:
            ...
            print(f"  [MUTATED] {os.path.basename(filepath)}: {purged_in_file} filas purgadas.")
    ...
    if total_purged > 0:
        ...
        if git_sentinel_commit(directory, mutated_files + [yaml_out], total_purged):
            print("[+] Bucle Ouroboros cerrado con éxito. Registro criptográfico fijado.")
        else:
            print("[-] Alerta: Los cambios se guardaron pero el commit del Sentinel falló.")
    else:
        print("[-] No se eliminaron filas. Las estructuras no coincidieron con el anclaje físico ^| ID |.")
