# C5-REAL: Convergencias Ontológicas Extendidas
import os
import glob
import yaml
import re
...
def get_artifact_path(filename: str) -> str:
    ...
    if os.path.exists(base_brain_dir):
        subdirs: list[str] = [os.path.join(base_brain_dir, d) for d in os.listdir(base_brain_dir) if os.path.isdir(os.path.join(base_brain_dir, d)) and not d.startswith(".")]
        if subdirs:
...
def tokenize(text: str) -> set[str]:
    ...
    stop_words: set[str] = {'para', 'como', 'este', 'esta', 'pero', 'todo', 'nada', 'algo'}
...
def main() -> None:
    ...
    artifact_csv_path: str = get_artifact_path("todas_redundancias.csv")
    artifact_md_path: str = get_artifact_path("redundancias_convergencias_extendido.md")
    
    os.makedirs(os.path.dirname(artifact_csv_path), exist_ok=True)
    
    with open(artifact_csv_path, 'w', encoding='utf-8', newline='') as fcsv:
        writer = csv.writer(fcsv)
        writer.writerow(["Score", "Entidad_A_ID", "Origen_A", "Entidad_B_ID", "Origen_B"])
        for score, e1, e2 in convergences:
            writer.writerow([f"{score:.4f}", e1['id'], e1['source'], e2['id'], e2['source']])
    
    with open(artifact_md_path, "w", encoding='utf-8') as f:
        f.write("# 🌀 Convergencias Extendidas (Top 1000)\n\n")
        f.write(f"> **Directiva C5-REAL (Ω6)**: Se exponen las primeras 1000 colisiones ontológicas ordenadas por similitud. El dataset completo ({len(convergences)} filas) está en `todas_redundancias.csv`.\n\n")
    ...
    sys.stdout.write(f"[+] Análisis extendido generado. CSV: {artifact_csv_path}\n")
