...
def main() -> None:
    ...
    with open(artifact_csv_path, "w", encoding="utf-8", newline="") as fcsv:
        ...
        writer.writerow(
            ["Score", "Entidad_A_ID", "Origen_A", "Entidad_B_ID", "Origen_B"]
        )
        for score, e1, e2 in convergences:
            writer.writerow(
                [f"{score:.4f}", e1["id"], e1["source"], e2["id"], e2["source"]]
            )

    with open(artifact_md_path, "w", encoding="utf-8") as f:
        f.write("# 🌀 Convergencias Extendidas (Top 1000)\n\n")
        f.write(
            f"> **Directiva C5-REAL (Ω6)**: Se exponen las primeras 1000 colisiones ontológicas ordenadas por similitud. El dataset completo ({len(convergences)} filas) está en `todas_redundancias.csv`.\n\n"
        )
