...
Interpreto **500** como **250 primitivas de modelado y síntesis + 250 invariantes de comprobación**.
...
Todos los siguientes se preservan bajo el isomorfismo correspondiente. Algunos solo se aplican cuando la estructura posee grafos, órdenes, pesos, dinámica, topología, etc.
...
6. **Verificar conmutación exacta:**
   \[
   \varphi(F(a_1,\ldots,a_n))
   =
   F'(\varphi(a_1),\ldots,\varphi(a_n)).
   \]
7. **Verificar relaciones en ambos sentidos:**
   \[
   R(\vec a)\iff R'(\varphi(\vec a)).
   \]
