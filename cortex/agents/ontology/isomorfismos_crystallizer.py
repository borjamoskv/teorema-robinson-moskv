...
def parse_isomorfismos() -> None:
    ...
    with open(md_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line_str.startswith("## ") or (
                line_str[0].isdigit()
                and ". " in line_str
                and not line_str.startswith("1. Cognición")
            ):
                ...
                if len(parts) == 2:
                    ...
                    if dom_a not in [
                        "humano",
                        "biológico/social (humano)",
                        "físico / mecánico (relevo 4x4)",
                        "biológico/social humano",
                    ]:
                        if current_category:
                            data[current_category].append(
                                {"domain_a": parts[0], "domain_b": parts[1]}
                            )
    ...
    sys.stdout.write(
        f"[*] Cristalizado 07_ISOMORFISMOS_BIO_SILICIO.md -> JSON/YAML ({total_mappings} mappings extraídos)\n"
    )
