        ...
        for line in f:
            ...
            if line_str.startswith("|") and not line_str.startswith("|-"):
                ...
                if not in_table:
                    ...
                    if len(parts) == len(headers) and parts[0].startswith(
                        ("PRIM", "INV", "ANTI", "RED", "VEC")
                    ):
...
def execute_ingest(conn: sqlite3.Connection, md_files: list[str]) -> int:
    ...
    for md_file in sorted(md_files):
        table_name: str = (
            os.path.basename(md_file).split("_", 1)[1].replace(".md", "").lower()
        )
...
def main() -> None:
    ...
    try:
        ...
        conn.close()
        sys.stdout.write(
            f"[+] ÉXITO. {total} Entidades Inyectadas en {DB_PATH} (Modo WAL).\n"
        )
    except sqlite3.OperationalError as e:
        err_msg: str = str(e)
        if "readonly" in err_msg or "read-only" in err_msg or "permission" in err_msg:
            sys.stdout.write(
                f"[!] ADVERTENCIA: {DB_PATH} es de solo lectura (Regla Σ4). Reencaminando a sidecar: {SIDECAR_PATH}\n"
            )
            conn_sidecar: sqlite3.Connection = init_db(use_sidecar=True)
            total_sidecar: int = execute_ingest(conn_sidecar, md_files)
            conn_sidecar.close()
            sys.stdout.write(
                f"[+] ÉXITO. {total_sidecar} Entidades Inyectadas en sidecar {SIDECAR_PATH} (Modo WAL).\n"
            )
        else:
            raise e
