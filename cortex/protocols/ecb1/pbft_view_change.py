...
# ❖ PBFT EMOJI VIEW-CHANGE ENGINE (C5-REAL) ❖
# Tolerancia a Líder Bizantino mediante Rotación de Vistas
...
class PBFTNetwork:
    ...
    def simulate_view_change_round(self):
        ...
        # El Primary actúa con malicia absoluta (Maximum Entropy Split-Brain)
        # Para evitar que las 3 réplicas alcancen quórum (2f=2), envía 3 payloads distintos.
        payloads = ["📦🧠⚡", "📦🧠💀", "📦🧠🩸"]
        target_idx = 0
        ...
        for i in range(N):
            ...
            if max_count < 2 * F:
                sys.stdout.write(f"    N{i} TIMEOUT :: Fallo de Quórum en Prepare ({max_count}/{2*F})\n")
            else:
                quorum_reached = True
        ...
        if not quorum_reached:
            ...
            for i in range(N):
                if i == primary: continue # Antiguo líder es excluido
            ...
            sys.stdout.write("\n  [ 5. NEW VIEW ]\n")
            # Nuevo líder espera 2f mensajes
            vc_count = len(view_changes[new_primary])
            if vc_count >= 2 * F:
                msg = f"{L_NWV}v{new_view}"
                sys.stdout.write(f"    N{new_primary} asume Liderazgo y broadcasts :: {msg}\n")
                self.view = new_view
...
if __name__ == "__main__":
    ...
    if "SUCCESS" in result:
        sys.stdout.write(f"El Líder Bizantino (N0) ha sido depuesto. La red opera ahora bajo la Vista {net.view} con Líder N{net.get_primary()}.\n")
