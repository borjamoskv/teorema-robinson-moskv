...
EMOJI_BY_STATE = {'IDLE': '📦', 'PROCESSING': '🧠', 'SUCCESS': '⚡', 'ERROR': '💀', 'RECOVERY': '🩸', 'TERMINAL': '📤'}
...
def append_event(origin_node: str, origin_sig: str, phase: str, state: Optional[str], payload: dict) -> dict:
    ...
    with DB_LOCK:
        ...
        try:
            ...
            unsigned_event = {'event_id': event_id, 'origin_node_id': origin_node, 'phase': phase, 'state': state, 'payload': payload, 'timestamp': timestamp, 'previous_hash': previous_hash}
...
async def create_event(request: Request, event: AuditEventInput, x_node_id: str=Header(...), x_node_signature: str=Header(...)):
