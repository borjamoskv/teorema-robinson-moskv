...
CLUSTER_KEYS: Dict[str, VerifyingKey] = {NODE_ID: public_key}
EMOJI_BY_STATE = {
    "IDLE": "📦",
    "PROCESSING": "🧠",
    "SUCCESS": "⚡",
...
def init_db() -> "Any":
    with connect_db() as conn:
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS cluster_keys (node_id TEXT PRIMARY KEY, public_key_hex TEXT NOT NULL)"
        )
        conn.execute(
            "INSERT OR IGNORE INTO cluster_keys (node_id, public_key_hex) VALUES (?, ?)",
...
def load_cluster_keys() -> "Any":
    with connect_db() as conn:
        ...
        for r in rows:
            CLUSTER_KEYS[r["node_id"]] = VerifyingKey.from_string(
                bytes.fromhex(r["public_key_hex"]), curve=SECP256k1
            )
...
def append_event(
    origin_node: str, origin_sig: str, phase: str, state: Optional[str], payload: dict
) -> dict:
    ...
    with DB_LOCK:
        ...
        try:
            ...
            event_hash = hashlib.sha256(
                canonical_json(unsigned_event).encode("utf-8")
            ).hexdigest()
    ...
    return {
        **unsigned_event,
        "hash": event_hash,
        "validator_signature": validator_signature_hex,
    }
...
def register_node(node_id: str = Query(...), pub_key_hex: str = Query(...)) -> "Any":
    try:
        ...
        with connect_db() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO cluster_keys (node_id, public_key_hex) VALUES (?, ?)",
                (node_id, pub_key_hex),
            )
            conn.commit()
...
async def create_event(
    ...
    if x_node_id not in CLUSTER_KEYS:
        raise HTTPException(
            403,
            f"Nodo {x_node_id} desconocido. Requiere registro previo en BFT Cluster.",
        )
    ...
    try:
        ...
        raise HTTPException(
            403, f"💀 Firma ECDSA rechazada (Spoofing detectado desde {x_node_id})"
        )
    state = STATE_BY_EMOJI.get(event.emoji) if event.emoji else event.state
    return {
        "accepted": True,
        "event": append_event(
            origin_node=x_node_id,
            origin_sig=x_node_signature,
            phase=event.phase,
            state=state,
