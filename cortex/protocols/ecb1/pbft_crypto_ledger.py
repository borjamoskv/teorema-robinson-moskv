...
class CryptoLedgerFusion:
    ...
    def setup_ledger(self):
        ...
        cur = self.conn.execute(
            "SELECT block_hash FROM pbft_crypto_ledger ORDER BY id DESC LIMIT 1"
        )
        row = cur.fetchone()
        if row:
    ...
    def write_block(self, phase, node_id, payload, signature):
        ...
        block_data = (
            f"{self.view}:{phase}:{node_id}:{payload}:{signature}:{self.last_hash}"
        )
    ...
    def run_crypto_consensus(self):
        ...
        for i in range(N):
            ...
            sys.stdout.write(
                f"    N{i} firma -> {sig_prp[:16]}... | BlockHash: {b_hash[:8]}\n"
            )
        ...
        for i in range(N):
            ...
            sys.stdout.write(
                f"    N{i} firma -> {sig_cmt[:16]}... | BlockHash: {b_hash[:8]}\n"
            )
        sys.stdout.write(
        ...
        fake_msg = f"{L_CMT}📦🧠💀"
        fake_sig = hmac.new(
            b"CLAVE_FALSA", fake_msg.encode(), hashlib.sha256
        ).hexdigest()
        try:
    ...
    def audit_chain(self):
        ...
        for r in reversed(rows):
            sys.stdout.write(
                f"  [{r[0].ljust(11)} N{r[1]}] :: Hash: {r[2][:16]}... <- Prev: {r[3][:16]}...\n"
            )
