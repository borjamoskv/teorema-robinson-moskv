...
import os
import secrets
from cortex.protocols.shadow_router.signing.ed25519_signer import Ed25519Signer, verify_signature, canonicalize_json
...
def test_ed25519_signing_and_verification():
    ...
    is_valid = verify_signature(public_key_hex, payload, sig_data['payload_hash'], sig_data['signature']['value'])
...
def test_ed25519_tampered_payload_fails():
    ...
    is_valid = verify_signature(public_key_hex, tampered_payload, sig_data['payload_hash'], sig_data['signature']['value'])
