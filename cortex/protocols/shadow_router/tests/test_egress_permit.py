...
def test_egress_gate_single_use():
    ...
    # Create valid DecisionReceipt payload
    decision_payload = {
        ...
        "shadow_policy": {
            ...
            "selected_shadow_routes": [
                {"route_id": "claude-3.7-sonnet", "conditional_inclusion_probability_basis_points": 10000}
            ]
        },
        "utility_predictions": {
            "selected_model_utility_basis_points": 8000,
            "shadow_candidates_utility_basis_points": {"claude-3.7-sonnet": 8500},
    ...
    # Generate signature using the mock private key hex
    # Real signature vector for Ed25519 or we bypass trust check by returning True in verify_envelope
    # For testing ease, let's mock verify_envelope to return True
    ...
    sig_block = SignatureBlock(
        algorithm="Ed25519",
        key_id="did:key:z6MkpTHR8VNsBxRcmStjecrxVCoVdPk2yFw6J22tCSZazn1x",
        value="signature_val",
