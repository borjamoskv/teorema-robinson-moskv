...
def test_paired_proxy_difference_estimation():
    ...
    estimate = estimate_shadow_proxy_difference(paired_differences_basis_points=diffs, shadow_inclusion_indicators=indicators, shadow_inclusion_propensities_basis_points=propensities, bootstrap_replicates=100)
...
def test_observed_proxy_regret_nonnegative():
    regret = calculate_observed_proxy_regret(primary_route_id='gemini-flash', primary_utility_basis_points=8000, shadow_utilities_basis_points={'claude-sonnet': 8500}, utility_spec_hash='sha256:util', evaluator_hash='sha256:eval')
    assert regret.observed_proxy_regret_basis_points == 500
    regret_best = calculate_observed_proxy_regret(primary_route_id='gemini-flash', primary_utility_basis_points=9000, shadow_utilities_basis_points={'claude-sonnet': 8500}, utility_spec_hash='sha256:util', evaluator_hash='sha256:eval')
