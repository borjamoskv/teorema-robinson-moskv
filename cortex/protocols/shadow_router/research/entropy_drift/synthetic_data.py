...
def generate_synthetic_cohort(size: int = 1000) -> List[Dict[str, Any]]:
    """
    Genera un log sintético con timestamps estrictos en UTC.
    Usado solo para probar que offline_backtest.py corre sin errores I/O,
    NO para validar la hipótesis predictiva real.
    """
    records = []
    base_time = datetime.utcnow()
    
    for i in range(size):
        # Avanza tiempo uniformemente para rellenar bins
        record_time = base_time + timedelta(minutes=i*10)
        
        has_label = random.random() > 0.1 # 90% coverage
