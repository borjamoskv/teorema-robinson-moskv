...
class EvaluatorBrierScore:
    ...
    def compute_brier(self, records: List[Dict[str, Any]]) -> Optional[float]:
        ...
        for record in records:
            if (
                "predicted_confidence" not in record
                or record["predicted_confidence"] is None
            ):
                continue
            if (
                "independent_label_is_correct" not in record
                or record["independent_label_is_correct"] is None
            ):
                continue
