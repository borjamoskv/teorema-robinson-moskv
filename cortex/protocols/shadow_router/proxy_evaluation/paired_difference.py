...
def estimate_shadow_proxy_difference(
    paired_differences_basis_points: np.ndarray,  # Delta_i = U_shadow - U_primary
    shadow_inclusion_indicators: np.ndarray,  # I_i = 1 if shadow executed
    shadow_inclusion_propensities_basis_points: np.ndarray,  # q_i = P(shadow_included | x_i, policy)
    confidence_level_basis_points: int = 9500,  # 95%
    bootstrap_replicates: int = 10000,
    weight_trim_threshold: float = 50.0,
    random_seed: int = 42
) -> ProxyDifferenceEstimate:
    """
    Estimates the mean paired proxy difference using the Hájek estimator.
    """
    ...
    ci = ConfidenceInterval(
        level_basis_points=confidence_level_basis_points,
        lower_basis_points=int(ci_lower),
        upper_basis_points=int(ci_upper),
        method="hajek_weighted_bootstrap"
    )
    
    return ProxyDifferenceEstimate(
        estimand="mean_paired_proxy_difference",
        estimate_basis_points=int(estimate),
        confidence_interval=ci,
        estimator="hajek_weighted_paired_difference",
        assumptions=[
            "shadow_inclusion_independent_of_outcome_given_features",
            "overlap",
            "no_interference_between_primary_and_shadow"
        ],
        sample_size=n,
        effective_sample_size=int(effective_n),
        max_weight_basis_points=int(max_weight * 10000),
        weight_trimming_applied=weight_trimming_applied
    )
