...
from cortex.protocols.shadow_router.protocol.schemas import (
    ReceiptEnvelope, SignatureBlock, DecisionReceipt, RoutingPolicy,
    ShadowPolicy, ShadowRouteSelection, UtilityPredictions, PrivacyEligibilityResult,
    ExecutionReceipt, ExecutionMetrics, TelemetrySource
)
...
class ShadowRouter:
    ...
    async def route_request(self, request_payload: str, metadata: Dict[str, Any]) -> ReceiptEnvelope:
        ...
        shadow_policy = ShadowPolicy(
            ...
            selected_shadow_routes=[shadow_selection] if metadata.get("shadow_eligible", True) else []
        )
