...
class EgressGate:
    ...
    def authorize_egress(self, decision_envelope: ReceiptEnvelope, route_id: str, purpose: str, validity_seconds: int=30) -> EgressPermit:
        ...
        now = datetime.now()
        permit = EgressPermit(issuer='did:web:router.cortex.internal', decision_signed_receipt_hash=decision_envelope.payload_hash, route_id=route_id, purpose=purpose, permit_id=str(uuid.uuid4()), issued_at=now, expires_at=now + timedelta(seconds=validity_seconds), single_use=True)
        return permit
