...
def create_ttft_measurement(
    ...
    if not request_started_at_ns <= first_byte_at_ns <= completed_at_ns:
        raise ValueError(
            "Invariantes temporales violados: t_start <= t_first_byte <= t_completed"
        )
    ...
    if streaming_enabled:
        if first_content_token_at_ns is None:
            raise ValueError(
                "first_content_token_at_ns es requerido si streaming está activo."
            )
        if not first_byte_at_ns <= first_content_token_at_ns <= completed_at_ns:
            raise ValueError(
                "Invariantes temporales violados en streaming: t_first_byte <= t_first_token <= t_completed"
            )
