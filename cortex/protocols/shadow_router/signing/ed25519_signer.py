...
# INV_CRYPTO_01: Strict Canonicalization Barrier
def canonicalize_json(payload: dict) -> bytes:
    """Canonicalize a JSON dictionary strictly following RFC 8785 rules."""
    return json.dumps(payload, separators=(',', ':'), sort_keys=True, ensure_ascii=False).encode('utf-8')

class Ed25519Signer:
    """
    C5-REAL Ed25519 Signer.
    Enforces INV_CRYPTO_01 by strictly signing over the canonicalized payload.
    """
    ...
    def sign_payload(self, payload: dict) -> dict:
        """
        Takes a JSON dictionary, computes the canonical hash, and signs it.
...
def verify_signature(public_key_hex: str, payload: dict, payload_hash: str, signature_b64: str) -> bool:
    """
    Verifies that the signature matches the payload and the hash.
    Fail-fast C5-REAL paradigm.
    """
    # 1. Verify Hash
    canonical_bytes = canonicalize_json(payload)
