...
def run_tests():
    try:
        ...
        assert len(node1.causal_hash) == 32, 'Violación criptográfica: Hash Blake2b no es válido.'
        node2 = calculate_exergy(20, 5.0)
        assert node2.exergy == 400.0, 'Violación termodinámica: Cálculo erróneo.'
        assert node1.causal_hash != node2.causal_hash, 'Colisión entrópica: Los hashes deben ser únicos.'
        with sqlite3.connect(DB_PATH, timeout=5.0) as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT COUNT(*) FROM exergy_ledger WHERE causal_hash IN (?, ?)', (node1.causal_hash, node2.causal_hash))
