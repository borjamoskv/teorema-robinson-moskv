...
def run_apoptosis(target_dir: str, execute: bool) -> None:
    ...
    for filepath, (defined, _) in file_map.items():
        ...
        if local_dead and prune_file(filepath, local_dead, execute):
            print(f"[-] {'Purgado' if execute else 'Simulado'}: {os.path.basename(filepath)} ({len(local_dead)} nodos: {', '.join(local_dead)})")
            pruned_files += 1
