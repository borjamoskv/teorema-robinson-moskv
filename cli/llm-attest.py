...
def verify_receipt(receipt_path: str):
    ...
    if sig_block.get('algorithm') == 'Ed25519':
        sig_valid = verify_ed25519(sig_block.get('key_id', '').replace('did:key:', ''), sig_block.get('value', ''), declared_hash.encode('utf-8'))
    else:
        print(f"ERROR: Algorithm {sig_block.get('algorithm')} is not implemented. Failing closed.")
        sig_valid = False
    report = {'receipt_id': receipt.get('receipt_id', 'unknown'), 'schema': receipt.get('schema', 'unknown'), 'payload_hash_valid': hash_valid, 'signature_valid': sig_valid, 'status': 'verified' if hash_valid and sig_valid else 'invalid'}
