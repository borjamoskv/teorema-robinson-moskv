...
def run_apoptosis(target_dir: str, execute: bool) -> None:
    ...
    print(f"[*] Detonando Apoptosis Ontológica (AST-Pruner) en {target_dir} | Modo: {mode}")
    ...
    if execute:
        compensators = {
            "APOPTOSIS_FILE_": lambda p: print(f"         [SAGA] Verificando purga idempotente para {p}")
        }
            ...
            if execute:
                def _effect():
                    if prune_file(filepath, local_dead, execute):
                        print(f"[-] Purgado: {os.path.basename(filepath)} ({len(local_dead)} nodos: {', '.join(local_dead)})")
                ...
                if prune_file(filepath, local_dead, execute):
                    print(f"[-] Simulado: {os.path.basename(filepath)} ({len(local_dead)} nodos: {', '.join(local_dead)})")
                pruned_files += 1
            
    if execute:
        print(f"[*] Apoptosis finalizada. Archivos mutados físicamente: {pruned_files}")
    else:
        print(f"[*] Simulación finalizada. Archivos infectados identificados: {pruned_files}. Usa --execute para purgar.")

def main():
    parser = argparse.ArgumentParser(description="C5-REAL Apoptosis Engine")
    parser.add_argument("dir", nargs="?", default=".", help="Directorio objetivo a analizar")
    parser.add_argument("--execute", action="store_true", help="Fuerza la mutación atómica en disco (Purga)")
