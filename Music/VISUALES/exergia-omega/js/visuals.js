...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Visibility gate — suspend canvas work when tab is hidden
  ...
  function renderLoop() {
    ...
    // Skip all draw work when page is hidden (background tab)
    if (!_pageVisible || !engine.initialized) return;
    ...
    if (engine.isPlaying) {
      ...
      const rmsLinear = Math.max(
        0.0,
        Math.min(1.0, Math.pow(10, rmsOutDb / 20)),
      );
      const peakLinear = Math.max(
        0.0,
        Math.min(1.0, Math.pow(10, peakOutDb / 20)),
      );
      ...
      const highEnergy =
        highEnd - highStart > 0 ? highSum / (highEnd - highStart) / 255.0 : 0.0;
      ...
      document.documentElement.style.setProperty(
        "--audio-high",
        highEnergy.toFixed(4),
      );
      document.documentElement.style.setProperty(
        "--audio-peak",
        peakLinear.toFixed(4),
      );
    ...
    if (
      ...
      for (let x = 0; x < wSpec; x++) {
        ...
        const idx = Math.max(
          0,
          Math.min(len - 1, Math.floor((freq / nyquist) * len)),
        );
    ...
    if (engine.isPlaying && freqDataL && freqDataR) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const y = Math.max(
          0,
          Math.min(hSpec, ((db + 60) / 60) * -hSpec + hSpec),
        );
    ...
    if (ctxShaper && shaperCanvas) {
      ...
      ctxShaper.beginPath();
      ctxShaper.moveTo(w / 2, 0);
      ctxShaper.lineTo(w / 2, h);
      ctxShaper.moveTo(0, h / 2);
      ctxShaper.lineTo(w, h / 2);
      ctxShaper.stroke();

      // Draw transfer curve based on active engine parameters
      ctxShaper.strokeStyle =
        engine.params.satMix > 0.05
          ? "rgba(255, 159, 28, 0.95)"
          : "rgba(43, 59, 229, 0.85)";
      ctxShaper.shadowColor =
        engine.params.satMix > 0.05 ? "#FF9F1C" : "#2B3BE5";
    ...
    if (ctxRadar && radarCanvas) {
      ...
      if (crossfeedMix > 0.05) {
        ...
        for (let s = 0; s <= segments; s++) {
          ...
          const waveAmp =
            Math.sin(t * Math.PI * 6 - Date.now() * 0.02 * (3.0 - delayMs)) *
            3.0 *
            crossfeedMix *
            (1.0 + rms);
      ...
      ctxRadar.lineTo(
        centerX + Math.cos(angle) * headRadius * 3,
        centerY + Math.sin(angle) * headRadius * 3,
      );
      ctxRadar.stroke();
    }
    ...
    if (ctxEq && eqCanvas && engine.initialized) {
      ...
      ctxEq.fillText(
        "1kHz",
        (Math.log10(1000 / 20) / Math.log10(20000 / 20)) * w + 3,
        h - 4,
      );
    ...
    exportBtn.addEventListener("click", async () => {
      ...
      const sourceName = engine.isPlaying
        ? document
            ...
            .innerText.replace("PLAYING: ", "")
            .replace("CARGANDO: ", "")
        : "OFFLINE / DRONE GENERATOR";
      const peakStr =
        lastPeakOutDbVal > -60
          ? `${lastPeakOutDbVal.toFixed(2)} dB`
          : "-inf dB";
...
`;
  ...
  function startRecording() {
    ...
    addP0Log(
      "Recording Started. Capturing WebGL + Mastered Audio...",
      "system",
    );
  }
  ...
  presetButtons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      addP0Log(
        `Morphing Vibe Profile to: ${presetName.toUpperCase()}...`,
        "system",
      );
    });
  });
