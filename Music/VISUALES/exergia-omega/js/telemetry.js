...
document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            ws.onmessage = (event) => {
                ...
                if (payload.metrics) {
                    ...
                    window.CORTEX_TELEMETRY.ontology = payload.metrics.ontology_similarity || (payload.metrics.cortisol_level ? 0.95 - (payload.metrics.cortisol_level * 0.5) : 0);
                    window.CORTEX_TELEMETRY.entropy = payload.metrics.structural_entropy || (window.CORTEX_TELEMETRY.cortisol * 2.5);
                    ...
                    if(hudOntology) hudOntology.innerText = `ONTOLOGY: ${window.CORTEX_TELEMETRY.ontology.toFixed(4)}`;
                    if(hudEntropy) hudEntropy.innerText = `ENTROPY: ${window.CORTEX_TELEMETRY.entropy.toFixed(4)}`;
                    if(hudOntologyHeader) hudOntologyHeader.innerText = `ONT: ${window.CORTEX_TELEMETRY.ontology.toFixed(2)}`;

                    // Occasional log feed injection
                    if (Math.random() < 0.05) {
                        if (window.CORTEX_TELEMETRY.cortisol > 0.7) {
                            addP0Log(`High Cortisol Detected (${window.CORTEX_TELEMETRY.cortisol.toFixed(2)}). Purging Stale Entelechy Nodes...`, "purge");
                        } else if (window.CORTEX_TELEMETRY.ontology > 0.9) {
                            addP0Log(`Ontological Convergence achieved: ${window.CORTEX_TELEMETRY.ontology.toFixed(4)}. Structural Integrity verified.`, "success");
                        } else {
                            addP0Log(`Swarm Thermodynamics: Annihilating Entropy [S=${window.CORTEX_TELEMETRY.entropy.toFixed(2)}]`, "system");
                        }
                    }
    ...
    function telemetryLoop() {
        ...
        let targetEntropy = window.CORTEX_TELEMETRY.connected ? 
            (window.CORTEX_TELEMETRY.entropy * 2.0 + window.CORTEX_TELEMETRY.cortisol * 5.0) : 0.5;
        ...
        // Smooth cortisol
        let targetCortisol = window.CORTEX_TELEMETRY.connected ? window.CORTEX_TELEMETRY.cortisol : 0.0;
        targetCortisol = Math.max(0.0, Math.min(1.0, targetCortisol));
        window.CORTEX_TELEMETRY.smoothedCortisol += (targetCortisol - window.CORTEX_TELEMETRY.smoothedCortisol) * 0.1;
    }
    telemetryLoop();
});
