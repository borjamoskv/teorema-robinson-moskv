...
document.addEventListener("DOMContentLoaded", () => {
    ...
    switch (e.key) {
        ...
        if (typeof engine !== "undefined") {
          if (engine.isPlaying) {
            ...
            if (typeof addP0Log === "function")
              addP0Log("SPACE: Playback Stopped", "system");
          } else {
            engine.startGenerator();
            const playBtn = document.getElementById("btn-play-generator");
            if (playBtn) playBtn.classList.add("active");
            if (typeof addP0Log === "function")
              addP0Log("SPACE: Generator Started", "system");
          }
        }
        break;
      ...
      case "B":
        if (typeof engine !== "undefined" && engine.initialized) {
          ...
          if (typeof addP0Log === "function")
            addP0Log(
              `B: Bypass ${nextBypass ? "ENGAGED" : "DISENGAGED"}`,
              "system",
            );
        }
        break;
      ...
      case "5": {
        ...
        if (presetBtns[idx]) {
          ...
          if (typeof addP0Log === "function") {
            addP0Log(
              `KEY[${e.key}]: Activated Vibe Preset "${presetBtns[idx].innerText}"`,
              "system",
            );
          }
        }
      ...
      case "Escape":
        if (
          shortcutsOverlay &&
          shortcutsOverlay.classList.contains("visible")
        ) {
          shortcutsOverlay.classList.remove("visible");
        }
        break;
  ...
  const _origPlayFile =
    typeof engine !== "undefined" ? engine.playFile.bind(engine) : null;
  ...
  if (waveformCanvas) {
    waveformCanvas.addEventListener("click", (e) => {
      ...
      if (typeof addP0Log === "function") {
        addP0Log(
          `Seeked to ${formatTime(seekTime)} / ${formatTime(waveformDuration)}`,
          "system",
        );
      }
    });
  ...
  function computeApproxLUFS() {
    ...
    for (let i = 0; i < framesPerWindow; i++) {
      const idx =
        (_lufsRingHead - 1 - i + LUFS_RING_CAPACITY) % LUFS_RING_CAPACITY;
  ...
  // Tab visibility gate (shared with main renderLoop)
  let _ctrlPageVisible = true;
  document.addEventListener('visibilitychange', () => {
    _ctrlPageVisible = !document.hidden;
  });
  ...
  function extendedRenderLoop() {
    ...
    if (_domLufsBarFill || _domLufsValue) {
      ...
      if (lufs > -70) {
        smoothedLufs =
          smoothedLufs === -Infinity ? lufs : smoothedLufs * 0.85 + lufs * 0.15;
      ...
      if (_domLufsBarFill) {
        const fillPct =
          smoothedLufs > -36
            ? Math.min(100, ((smoothedLufs + 36) / 36) * 100)
            : 0;
    ...
    if (_domSpectralTilt) {
      ...
      if (newClass !== _lastTiltClass) {
        ...
        const titles = {
          "tilt-bright": "Spectral Tilt: BRIGHT",
          "tilt-dark": "Spectral Tilt: DARK",
          "tilt-balanced": "Spectral Tilt: BALANCED",
        };
