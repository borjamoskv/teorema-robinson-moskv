...
class MasteringEngine {
  constructor() {
    ...
    this.params = {
      ...
      exciterOdd: 0.1, // Chebyshev odd harmonic level (0-1)
    ...
    // Pre-allocate shaper curve buffer ONCE (44100 floats = 176KB)
    // This eliminates the largest per-call allocation in the hot path.
    this._shaperCurve = new Float32Array(44100);

    // Ledger hash throttle
    this._lastLedgerHash = 0;
  }
  ...
  init() {
    ...
    document.getElementById("sample-rate-metric").innerText =
      `SR: ${(this.ctx.sampleRate / 1000).toFixed(1)} kHz`;
    ...
    if (!this.modulateInterval) {
      this.modulateInterval = setInterval(
        () => this.modulateFromTelemetry(),
        100,
      );
    }
  }
  ...
  updateParam(param, value) {
    ...
    switch (param) {
      ...
      case "sideHp":
        ...
        break;
      case "gainBoost":
        this.boostGainNode.gain.setValueAtTime(
          Math.pow(10, this.params.gainBoost / 20),
          now,
        );
        break;
      case "ceiling":
      ...
      case "limiterRelease":
        this.limiter.release.setValueAtTime(
          this.params.limiterRelease / 1000,
          now,
        );
        break;
    }
  ...
  updateShaperCurve() {
    ...
    const curve = this._shaperCurve;
    const n_samples = curve.length;
  ...
  modulateFromTelemetry() {
    ...
    if (window.CORTEX_TELEMETRY && window.CORTEX_TELEMETRY.connected) {
      ...
      // Temporarily override params for telemetry-driven modulation
      const origDrive = this.params.satDrive;
      const origOdd = this.params.exciterOdd;
      const origEven = this.params.exciterEven;

      this.params.satDrive = origDrive + entropy * 0.5 + cortisol * 1.2;
      this.params.exciterOdd = origOdd + entropy * 0.05;
      this.params.exciterEven = origEven + entropy * 0.02 + cortisol * 0.05;

      // Reuse updateShaperCurve — no duplicate allocation
      this.updateShaperCurve();

      // Restore original params (modulation is audio-only, not ledger state)
      this.params.satDrive = origDrive;
      this.params.exciterOdd = origOdd;
      this.params.exciterEven = origEven;
    }
  }
  ...
  startGenerator() {
    ...
    this.oscDrone.frequency.value = parseFloat(
      document.getElementById("drone-freq").value,
    );
  ...
  stop() {
    ...
    if (this.sourceNode) {
      try {
        ...
        console.warn("[DSP] sourceNode stop error:", e);
      }
      this.sourceNode = null;
    }
    ...
    if (this.oscDrone) {
      try {
        ...
        console.warn("[DSP] oscDrone stop error:", e);
      }
      this.oscDrone = null;
    }
    if (this.oscHarmonic1) {
      try {
        ...
        console.warn("[DSP] oscHarmonic1 stop error:", e);
      }
      this.oscHarmonic1 = null;
    }
    if (this.oscHarmonic2) {
      try {
        ...
        console.warn("[DSP] oscHarmonic2 stop error:", e);
      }
      this.oscHarmonic2 = null;
    }
    if (this.noiseNode) {
      try {
        ...
        console.warn("[DSP] noiseNode stop error:", e);
      }
      this.noiseNode = null;
    }
    if (this.lfoNode) {
      try {
        ...
        console.warn("[DSP] lfoNode stop error:", e);
      }
      this.lfoNode = null;
    }
  ...
  // Cryptographic validation hash — throttled to max 5 Hz to avoid
  // thrashing during lerp (was called 29+ times per preset morph)
  updateLedgerHash() {
    const now = performance.now();
    if (now - this._lastLedgerHash < 200) return; // 200ms gate
    this._lastLedgerHash = now;
