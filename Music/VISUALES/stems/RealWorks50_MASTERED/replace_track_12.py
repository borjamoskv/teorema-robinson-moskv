#!/usr/bin/env python3
"""
BANDCAMP TRACK REPLACER - Track 012 (Last Time)
"""
import os
import time
import subprocess
...
TRACK_PATH = os.path.expanduser("$CORTEX_ROOT/Music/VISUALES/stems/RealWorks50_MASTERED/012 - Borja Moskv - Last Time_MASTERED.flac")
...
def main():
    ...
    print("[DOM] Clicking replace on track 012...")
    execute_js("""
        var links = document.querySelectorAll('a.replace');
        if (links[11]) {
            links[11].scrollIntoView({behavior: 'instant', block: 'center'});
            links[11].click();
        }
    """)
    ...
    print("[DONE] Track 012 replacement triggered.")
