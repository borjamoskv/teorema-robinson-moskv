#!/usr/bin/env python3
"""
BANDCAMP TRACK REPLACER - Track 011 (The Chauffeur)
"""
import os
import time
import subprocess
...
TRACK_PATH = os.path.expanduser("$CORTEX_ROOT/Music/VISUALES/stems/RealWorks50_MASTERED/011 - Borja Moskv - The Chauffeur_MASTERED.flac")
...
def main():
    ...
    print("[DOM] Clicking replace on track 011...")
    execute_js("""
        var links = document.querySelectorAll('a.replace');
        if (links[10]) {
            links[10].scrollIntoView({behavior: 'instant', block: 'center'});
            links[10].click();
        }
    """)
    ...
    print("[DONE] Track 011 replacement triggered.")
