#!/usr/bin/env python3
"""
BANDCAMP TRACK REPLACER - Track 045 (A cualquier otra parte)
"""
import os
import time
import subprocess
...
TRACK_PATH = os.path.expanduser("$CORTEX_ROOT/Music/VISUALES/stems/RealWorks50_MASTERED/045 - Borja Moskv - A cualquier otra parte_MASTERED.flac")
...
def main():
    ...
    print("[DOM] Clicking replace on track 045...")
    execute_js("""
        var links = document.querySelectorAll('a.replace');
        if (links[44]) {
            links[44].scrollIntoView({behavior: 'instant', block: 'center'});
            links[44].click();
        }
    """)
    ...
    print("[DONE] Track 045 replacement triggered.")
